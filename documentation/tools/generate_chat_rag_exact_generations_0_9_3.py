...
def _compatibility_key(
    config: dict[str, Any], manifest: dict[str, Any], candidate: dict[str, Any], locale: str
) -> dict[str, Any]:
    values = {
...
def _validate_chunks(
    ...
    if (
        not isinstance(manifest.get("source_manifest_sha256"), str)
        or len(manifest["source_manifest_sha256"]) != 64
    ):
...
def _cache_entry(
    cache_root: Path, compatibility: dict[str, Any], passage: str
) -> tuple[Path, Path]:
...
def _read_cached_vector(
    cache_root: Path, compatibility: dict[str, Any], passage: str, dimension: int
) -> np.ndarray | None:
    ...
    try:
        ...
        return None
    if (
        metadata.get("compatibility_sha256")
        != hashlib.sha256(_canonical_json(compatibility)).hexdigest()
    ...
    if (
        ...
        or not np.isclose(np.linalg.norm(vector), 1.0, atol=1e-4)
    ):
        return None
    return vector.copy()
...
def _write_cached_vector(
    cache_root: Path, compatibility: dict[str, Any], passage: str, vector: np.ndarray
) -> None:
    vector_path, metadata_path = _cache_entry(cache_root, compatibility, passage)
...
def _generation_id(
    config: dict[str, Any], manifest: dict[str, Any], locale_entries: list[dict[str, Any]]
) -> str:
    identity = {
...
def validate_generation(root: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    for entry in manifest["locales"]:
        ...
        if (
            ...
            or _sha256(locale_manifest_path) != entry["manifest_sha256"]
        ):
        ...
        if (
            set(compatibility) != set(config["compatibility_key"])
            or compatibility.get("locale") != entry["locale"]
        ):
        ...
        if (
            metadata.get("locale") != entry["locale"]
            or not isinstance(chunks, list)
            or len(chunks) != rows
        ):
            raise GenerationError("generation chunk metadata shape mismatch")
        if any(
            set(chunk) != set(matrix["metadata_fields"])
            for chunk in chunks
...
def build_generation(
    ...
    try:
        ...
        for locale in config["input"]["locales"]:
            ...
            def report(
                completed: int, total: int, cache_hits: int, current_locale: str = locale
            ) -> None:
                ...
                if last_percent >= 0 and (
                    ...
                    or (
                        percent != 100
                        and percent < last_percent + config["progress"]["update_percent"]
                    )
                ):
                    return
            ...
            vectors, misses = _vectors_for_locale(
                ...
                compatibility,
                cache_root,
                config["matrix"]["dimension"],
                encode,
                report,
            )
            cache_misses[locale] = misses
            locale_entries.append(
                _write_locale(stage, locale, grouped[locale], vectors, compatibility, config)
            )
            print(
        ...
        (stage / config["matrix"]["root_manifest_file_name"]).write_bytes(
            _canonical_json(root_manifest)
        )
        ...
        _write_json_atomic(
            output_root / config["matrix"]["active_pointer_file_name"],
            {
                "generation_id": generation_id,
                "manifest_sha256": _sha256(final / config["matrix"]["root_manifest_file_name"]),
            },
        )
        return {
            "generation_id": generation_id,
...
def run(
    ...
    intra_op = (
        os.cpu_count() or 1
        if runtime["intra_op_threads"] == "all_available_logical_threads"
        else runtime["intra_op_threads"]
    )
    benchmark_config = {
        "runtime": {
