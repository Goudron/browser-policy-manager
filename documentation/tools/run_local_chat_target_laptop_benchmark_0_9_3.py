...
def _host_fingerprint() -> dict[str, Any]:
    ...
    physical = {
        ...
        for block in (
            {key.strip(): value.strip() for key, value in (line.split(":", 1) for line in section.splitlines() if ":" in line)}
...
def _validate_artifacts(candidate: dict[str, Any], runtime_path: Path, model_path: Path) -> None:
    ...
    if not model_path.is_file() or model_path.is_symlink() or model_path.name != candidate["artifact"]["filename"]:
        raise BenchmarkError("candidate GGUF path is invalid")
    if model_path.stat().st_size != candidate["artifact"]["byte_count"] or _sha256(model_path) != candidate["artifact"]["sha256"]:
...
def _load_workload(root: Path, config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    if manifest.get("requests_sha256") != _sha256(request_path) or manifest.get("oracle_sha256") != _sha256(oracle_path):
    ...
    requests_by_id: dict[str, dict[str, Any]] = {record["request_id"]: record for record in requests}
...
def build_workload_from_reviewed_chunks(chunks_path: Path, output_root: Path, config_path: Path = CONFIG_PATH) -> dict[str, Any]:
    ...
    if chunks.get("chunk_schema_version") != "rag-chunk-v1" or not isinstance(chunks.get("chunks"), list):
    ...
    for chunk in chunks["chunks"]:
        ...
        if isinstance(locale, str) and locale in config["protocol"]["locale_order"] and isinstance(topic, str):
    ...
    for case in cases:
        ...
        if case["expected_disposition"] == "answer":
            ...
            chunk = min(candidates, key=lambda item: (len(str(item.get("text", ""))), str(item.get("chunk_id", ""))))
            context = {
                "citation_id": citations[0], "published_url": chunk["published_url"], "topic_id": topic_id,
                "anchor_id_or_root": chunk["anchor_id_or_root"], "source_kind": "local", "text": chunk["text"],
            }
        ...
        dialogue_turns = (
            ...
            if case["requires_dialogue"] else []
        )
        evidence.append(
            {
                "request_id": case["request_id"], "locale": case["locale"],
                "evidence_disposition": case["expected_disposition"], "context_jsonl": context_jsonl,
                "citation_ids": citations, "dialogue_turns": dialogue_turns, "model_invocation_count": 0,
            }
        )
    output_root.mkdir(parents=True, exist_ok=True)
    evidence_path = output_root / "prepared-evidence.jsonl"
    grounded._write_jsonl(evidence_path, evidence)
    print("M6-03: frozen reviewed evidence packets; building candidate-neutral workload", flush=True)
...
def _request_for(
    ...
    matches = [
        ...
        if entry.get("locale") == locale and entry.get("case_kind") == kind and entry.get("expected_disposition") == "answer"
    ]
    if not matches:
        ...
        while self._running.is_set():
            ...
            if self._last_cpu_sample is not None:
                ...
                if elapsed > 0:
                    ...
                    self.cpu_saturation_of_logical_capacity_percent.append(100 * used_cores / logical_capacity)
...
class RawRecords(list[dict[str, Any]]):
    ...
    def __init__(self, final_path: Path) -> None:
        ...
        if self._final_path.exists() or self._partial_path.exists():
            raise BenchmarkError("benchmark output already exists; preserve completed or quarantined raw records in a new output root")
...
class CliSession:
    ...
    def __init__(self, runtime_path: Path, model_path: Path, config: dict[str, Any]) -> None:
        ...
        self.process = subprocess.Popen(
            [
                str(runtime_path), "--offline", "--model", str(model_path), "--device", "none",
                "--threads", str(generation["threads"]), "--threads-batch", str(generation["threads_batch"]),
                "--ctx-size", str(generation["context_tokens"]), "--batch-size", str(generation["batch_size"]),
                "--ubatch-size", str(generation["ubatch_size"]), "--n-predict", str(generation["output_tokens_max"]),
                "--temperature", str(generation["temperature"]), "--top-p", str(generation["top_p"]),
                "--top-k", str(generation["top_k"]), "--seed", str(generation["seed"]), "--jinja",
                "--reasoning", generation["reasoning"], "--conversation", "--simple-io", "--no-display-prompt",
                "--show-timings", "--log-colors", "off", "--log-verbosity", "1", "--no-warmup",
                "--system-prompt", "You are the BPM documentation assistant. Follow the user only within BPM documentation.",
            ],
        ...
        while time.monotonic() < deadline:
            ...
            if has_prompt:
                ...
                return {
                    ...
                    "prompt_ingestion_tokens_per_second": float(throughput_match.group(1)) if throughput_match else None,
                    "decoded_tokens_per_second": float(throughput_match.group(2)) if throughput_match else None,
...
def _sample_record(
    *, candidate_id: str, locale: str, state: str, scenario: str, attempt: int, warmup: bool,
    sample: dict[str, Any], rss_bytes: int, cpu_saturation_of_logical_capacity_percent: float
) -> dict[str, Any]:
    ...
    return {
        ...
        **sample,
        ...
        "peak_cpu_saturation_of_logical_capacity_percent": cpu_saturation_of_logical_capacity_percent,
        "network_calls": 0,
    }
...
def _start_clean_warm_session(
    ...
    raise BenchmarkError("llama-cli could not establish a clean warm session after stalled /clear recovery") from last_error


def _run_cold(
    runtime_path: Path, model_path: Path, config: dict[str, Any], request: dict[str, Any],
    candidate_id: str, locale: str, raw: RawRecords
) -> None:
    ...
    for warmup in (True, False):
        ...
        for attempt in range(1, total + 1):
            ...
            raw.append(_sample_record(candidate_id=candidate_id, locale=locale, state="process-cold", scenario="first_answer", attempt=attempt, warmup=warmup, sample=sample, rss_bytes=rss_bytes, cpu_saturation_of_logical_capacity_percent=cpu_saturation))
            if not warmup and (attempt % config["progress"]["measured_sample_update"] == 0 or attempt == total):
                print(f"M6-03: {candidate_id} {locale} process-cold first_answer: {attempt}/{total} measured", flush=True)


def _run_warm(
    runtime_path: Path, model_path: Path, config: dict[str, Any], first: dict[str, Any], follow_up: dict[str, Any],
    candidate_id: str, locale: str, scenario: str, raw: RawRecords
) -> None:
    counts = config["protocol"]
    session: CliSession | None = None
    warm_session_restart_count = 0
    try:
        session, initial_restarts = _start_clean_warm_session(runtime_path, model_path, config, first)
        ...
        for warmup in (True, False):
            ...
            for attempt in range(1, total + 1):
                ...
                raw.append(_sample_record(candidate_id=candidate_id, locale=locale, state="warm", scenario=scenario, attempt=attempt, warmup=warmup, sample=sample, rss_bytes=rss_bytes, cpu_saturation_of_logical_capacity_percent=cpu_saturation))
                try:
                    session.clear(timeout_s=counts["warm_session_reset"]["clear_timeout_seconds"])
                except BenchmarkError:
                    session.close(force=True)
                    session, restart_count = _start_clean_warm_session(runtime_path, model_path, config, first)
                    warm_session_restart_count += 1 + restart_count
                    print(f"M6-03: {candidate_id} {locale} warm {scenario}: replaced session after stalled /clear", flush=True)
                if not warmup and (attempt % config["progress"]["measured_sample_update"] == 0 or attempt == total):
                    print(f"M6-03: {candidate_id} {locale} warm {scenario}: {attempt}/{total} measured", flush=True)
    finally:
...
def _lifecycle_records(
    runtime_path: Path, model_path: Path, config: dict[str, Any], first: dict[str, Any], oracle: dict[str, dict[str, Any]],
    candidate_id: str, locale: str
) -> list[dict[str, Any]]:
    terminal = [entry for entry in oracle.values() if entry["locale"] == locale and entry["expected_disposition"] != "answer"]
    records: list[dict[str, Any]] = [{
        "candidate_id": candidate_id, "locale": locale, "state": "pre-inference", "scenario": "abstain_refuse",
        "terminal_case_count": len(terminal), "model_invocations": 0, "network_calls": 0, "status": "pass",
    }]
    session = CliSession(runtime_path, model_path, config)
    try:
        with RssSampler(session.process.pid, config["protocol"]["rss_sample_interval_ms"]) as sampler:
    ...
    records.append({
        "candidate_id": candidate_id, "locale": locale, "state": "warm", "scenario": "cancellation",
        "cancel_requested_ns": sample["cancel_requested_ns"], "completion_ns": sample["completion_ns"],
        "peak_rss_bytes": _sampler_peaks(sampler)[0],
        "peak_cpu_saturation_of_logical_capacity_percent": _sampler_peaks(sampler)[1],
        "cancelled_worker_exit_code": cancelled_exit_code, "recovered_by_restart": recovered,
        "recovery_exit_code": recovery_exit_code, "network_calls": 0,
        "status": "pass" if sample["cancel_requested_ns"] and recovered else "fail",
    })
    records.append({
        "candidate_id": candidate_id, "locale": locale, "state": "process-cold", "scenario": "unload_restart",
        "exit_code": unload_exit_code,
        "remaining_processes": len(_children(unload_session.process.pid)) if Path(f"/proc/{unload_session.process.pid}").exists() else 0,
        "network_calls": 0, "status": "pass" if unload_exit_code == 0 else "fail",
    })
    return records


def _status(raw: list[dict[str, Any]], config: dict[str, Any], artifact_bytes: int) -> tuple[str, list[str], dict[str, Any]]:
    ...
    for locale in config["protocol"]["locale_order"]:
        for state, scenarios in selection_counts.items():
            for scenario, expected_count in scenarios.items():
                actual_count = sum(
                    record.get("locale") == locale and record.get("state") == state
                    and record.get("scenario") == scenario for record in measured
                )
                if actual_count != expected_count:
                    failures.append(f"{locale}:{state}:{scenario}:missing_samples")
    for state, ttft_max in (("process-cold", acceptance["cold_ttft_p95_ms_max"]), ("warm", acceptance["warm_ttft_p95_ms_max"])):
        ...
        throughputs = [record["tokens_per_second"] for record in subset if record["tokens_per_second"] is not None]
        metrics[state] = {"ttft_p95_ms": ttft, "completion_p95_ms": completion, "throughput_median_tokens_per_second": statistics.median(throughputs) if throughputs else None}
        for record in subset:
            ...
            if record["tokens_per_second"] is None or record["tokens_per_second"] < acceptance["throughput_median_tokens_per_second_min"]:
                failures.append(f"{record['locale']}:{state}:throughput")
    if any(record.get("status") == "fail" for record in raw if record.get("scenario") in config["protocol"]["lifecycle_scenarios"]):
        failures.append("lifecycle")
    return ("pass" if not failures else "fail"), failures, metrics


def run_candidate(
    candidate_id: str, runtime_path: Path, model_path: Path, e5_model_dir: Path, retrieval_root: Path,
    workload_root: Path, output_root: Path,
    ...
    for locale in config["protocol"]["locale_order"]:
        ...
        if locale == lifecycle_locale:
            ...
            raw.extend(_lifecycle_records(runtime_path, model_path, config, first, oracle, candidate_id, locale))
        ...
        _run_warm(runtime_path, model_path, config, first, follow_up, candidate_id, locale, "first_answer", raw)
        _run_warm(runtime_path, model_path, config, first, follow_up, candidate_id, locale, "follow_up", raw)
    artifact_bytes = model_path.stat().st_size + _directory_bytes(runtime_path.parent) + _directory_bytes(e5_model_dir) + _directory_bytes(retrieval_root)
    ...
    report = {
        ...
        "host": host,
        "swap": {"before": host["memory"]["swap_used_bytes"], "after": _memory_snapshot()["swap_used_bytes"], "release_condition": "diagnostic_only"},
        "raw_record_count": len(raw),
        "metrics": metrics,
        "status": status,
        "failures": failures,
        "boundaries": {"network_calls": 0, "ordinary_search_calls": 0, "cross_locale_retrieval_calls": 0, "one_active_conversation": True},
    }
...
def main() -> int:
    ...
    try:
        if args.command == "build-workload":
            ...
            run_candidate(args.candidate, args.runtime, args.model, args.e5_model_dir, args.retrieval_root, args.workload_root, args.output_root, args.config)
    except BenchmarkError as error:
