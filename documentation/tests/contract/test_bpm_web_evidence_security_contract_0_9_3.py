...
def test_m9_05_freezes_cancellation_rate_privacy_and_no_route_boundaries() -> None:
    ...
    assert (
        rate_limit["requests_per_session_max"] == web_evidence.WEB_EVIDENCE_MAX_REQUESTS_PER_SESSION
    )
    ...
    assert contract["privacy_and_failure_isolation"]["offline_test_policy"].startswith(
        "all security tests"
    )
