...
def test_review_declares_m6_03_scope_honest_result_and_blockers() -> None:
    review = _json(REVIEW)
    ...
    assert review["status"] == "accepted-with-release-blockers"
    assert review["summary"] == {
        "section_review_count": 10,
        "reviewed_topic_count": 49,
        "task_topic_count": 45,
        "concept_topic_count": 4,
        "procedure_pass_topic_count": 36,
        "procedure_blocked_topic_count": 13,
        "release_blocker_count": 2,
        "result": (
            "Current operations, update, health, troubleshooting, integration, and "
            "production-boundary procedures agree with repository behavior under deterministic "
            "simulation. Source-install evidence and 0.9.1 version wording remain explicit "
            "release blockers."
        ),
    }
    assert {item["finding_id"] for item in review["release_blockers"]} == {
...
def test_review_normalization_resolves_protocol_fields_and_evidence() -> None:
    ...
    for section in review["section_reviews"]:
        ...
        assert section["review_disposition"] in {"pass", "blocked"}
...
def test_source_install_sections_remain_blocked_until_exact_commands_and_evidence_exist() -> None:
    review = _json(REVIEW)
    sections = _sections()
    blocker = next(
        item for item in review["release_blockers"]
        if item["finding_id"] == "ADMIN091-SOURCE-INSTALL-EVIDENCE"
    )
    source_install_topics = (
        sections["linux-source-deployment"]["topic_ids"]
        + sections["windows-wsl-source-deployment"]["topic_ids"]
    )
    current_source = "\n".join(_source("en", topic_id) for topic_id in source_install_topics)

    assert sections["linux-source-deployment"]["review_disposition"] == "blocked"
    assert sections["windows-wsl-source-deployment"]["review_disposition"] == "blocked"
    assert blocker["affected_topic_count"] == 13
    assert blocker["closure_tasks"] == []
    assert blocker["closure_evidence"] == (
        "docs/architecture/linux-source-install-validation-0.9.1.json must be updated with five "
        "clean-target passes before release."
    )
    ...
    assert "No clean-host Linux distribution" in " ".join(review["unverified_boundaries"])


def test_version_drift_is_recorded_for_every_current_administrator_topic() -> None:
    review = _json(REVIEW)
    blocker = next(
        item for item in review["release_blockers"]
    ...
    assert blocker["closure_tasks"] == ["BPM091-M12-02"]
    assert len(topic_ids) == 44
    for locale in LOCALES:
        for topic_id in topic_ids:
            assert 'product="bpm-0-9-0"' in _source(locale, topic_id)
    assert "BPM 0.9.0" in "\n".join(_source("en", topic_id) for topic_id in topic_ids)
...
def test_review_keeps_production_and_recovery_claims_bounded() -> None:
    ...
    assert all(
        ...
        if section_id not in {"linux-source-deployment", "windows-wsl-source-deployment"}
    )
