        ...
        evidence.append(
            {
                ...
                "context_jsonl": (
                    json.dumps({"citation_id": case["expected_citation_ids"][0]})
                    if is_answer
                    else ""
                ),
...
def _responses_and_reviews(workload_root: Path, candidate_id: str) -> tuple[list[dict], list[dict]]:
    oracle = [
        json.loads(line)
        for line in (workload_root / "oracle.jsonl").read_text(encoding="utf-8").splitlines()
    ]
    responses: list[dict] = []
    reviews: list[dict] = []
    for case in oracle:
...
def test_harness_builds_candidate_neutral_workload_and_rejects_unsupported_claims(
    ...
    assert (
        manifest["ordinary_search_calls"]
        == manifest["cross_locale_retrieval_calls"]
    ...
    assert all(
        metrics["terminal_disposition_rate"] == 1.0 for metrics in report["per_locale"].values()
    )
