...
EVIDENCE_ROOT = (
    REPOSITORY_ROOT
    / "documentation/evidence/live-source-install/0.9.1/m11-03-harness-20260714"
)
MANIFEST = EVIDENCE_ROOT / "run-manifest.json"
INVENTORY = EVIDENCE_ROOT / "retained-inventory.json"
HARNESS_CONFIG = (
    REPOSITORY_ROOT / "documentation/config/live-source-install-harness-0.9.1.json"
)
...
def test_harness_evidence_is_accepted_and_contract_backed() -> None:
    ...
    assert evidence["contracts"]["harness"] == HARNESS_CONFIG.relative_to(
        REPOSITORY_ROOT
    ).as_posix()
...
def test_mint_image_is_signed_iso_derived_and_identity_verified() -> None:
    ...
    assert mint["rootfs_identity"] == self_test["identity"] == {
        "ID": "linuxmint",
        "VERSION_ID": "22.3",
        "VERSION_CODENAME": "zena",
    }
    assert mint["numeric_owner_tar_sha256"] == mint["image_layer_digest"].removeprefix(
        "sha256:"
    )
