...
def test_relevance_benchmark_contract_locks_artifacts_and_non_selection_boundary() -> None:
    ...
    assert config["artifacts"]["pagefind"] == {
        ...
        "linux_x64_package_sha256": (
            "3c8dcfdaa69946113e0257be05bdef57297c7f1a562aa168536ff7c5d4590660"
        ),
        "source": "https://www.npmjs.com/package/pagefind/v/1.5.2",
    }
    assert config["artifacts"]["meilisearch_ce"]["version"] == "1.45.1"
    assert (
        config["artifacts"]["meilisearch_ce"]["linux_amd64_binary_sha256"]
        == "35986cba02cc4c9a2f79b4f85be8c2bc0013989c208410e6cfea85b1fdc3d708"
    )
...
def test_compact_index_keeps_only_canonical_and_reviewed_alias_forms() -> None:
    ...
    profile_en = next(
        ...
        if document["document_id"] == "bpm093-prototype:en:profile-library"
    )
    profile_ru = next(
        document
        for document in documents
        if document["document_id"] == "bpm093-prototype:ru:profile-library"
    )
    ...
    assert (
        benchmark._current_control_topics(
