...
def test_container_adapter_is_visible_and_documented_commands_remain_source_owned() -> None:
    ...
    assert "preserving the activated environment" in ownership["runtime_rule"]
    assert "stop waiting immediately" in ownership["runtime_rule"]
...
def test_evidence_contract_covers_commands_output_exit_inspect_and_inventory() -> None:
    ...
    tool = TOOL.read_text(encoding="utf-8")
    assert 'command.add_argument("--backlog-item", required=True' in tool
    assert '"backlog_item": args.backlog_item' in tool
