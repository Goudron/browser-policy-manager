...
BUILD_TOOL = ROOT / "documentation/tools/build_docs.py"
...
def test_pdf_generator_and_make_targets_preserve_the_delivery_boundary() -> None:
    ...
    assert "PDF source: {locale}/{guide_id}" in tool
    assert "PDF render: {locale}/{guide_id}" in tool
...
def test_chinese_pdf_navigation_titles_do_not_mix_fallback_font_metrics() -> None:
    """Keep Chinese PDF table-of-contents labels in one Unicode font family."""
    ...
    for topic in sorted((DITA_ROOT / "zh-CN").rglob("*.dita")) + sorted(
        ...
        for tag in ("title", "navtitle"):
            for element in root.findall(f".//{tag}"):
                assert not re.search(r"[A-Za-z]", "".join(element.itertext())), topic
