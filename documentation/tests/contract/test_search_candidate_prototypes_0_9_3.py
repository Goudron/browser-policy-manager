...
def test_candidate_prototype_contract_has_the_same_shortlist_and_a_non_execution_boundary() -> None:
    ...
    assert "not installed, downloaded, started, or benchmarked" in config["prototype_boundary"]["vendor_execution"]
    assert "not Pagefind or Meilisearch relevance results" in config["prototype_boundary"]["result_meaning"]
    ...
    for locale in LOCALES:
        ...
        for document in locale_documents:
            ...
            assert document["source_weights"] == {
                "exact_identifier": 1000, "title": 180, "alias": 140,
                "heading": 80, "body": 20, "bounded_typo": 12, "recency": 0,
            }
...
@pytest.mark.parametrize("candidate_id", ("current-static-control", "pagefind-1.5.2", "meilisearch-ce-1.45.1"))
def test_every_adapter_has_stable_normalized_results_for_identifiers_aliases_and_facets(candidate_id: str) -> None:
    ...
    filtered = prototypes.query_candidate(candidate_id, "en", "validation", {"api_area": ["validation"]})
    ...
    assert [result["topic_id"] for result in filtered["results"]] == ["admin-task-validate-firefox-policies-json"]
