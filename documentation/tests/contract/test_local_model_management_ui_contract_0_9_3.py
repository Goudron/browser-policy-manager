...
def test_m10_03a_keeps_model_lifecycle_separate_from_chat_enablement() -> None:
    ...
    assert contract["status"] == "implemented-explicit-release-model-management-with-reviewed-post-install-activation"
    ...
    assert 'operation.state === "failed" ? messages.actions.retry : messages.actions.install' in script
