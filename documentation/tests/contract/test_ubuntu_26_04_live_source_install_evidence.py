...
EVIDENCE_ROOT = (
    REPOSITORY_ROOT
    / "documentation/evidence/live-source-install/0.9.1/m11-04-ubuntu-source-install-20260714"
)
...
def test_ubuntu_live_install_is_accepted_against_an_immutable_091_ref() -> None:
    ...
    assert evidence["backlog_item"] == "BPM091-M11-04"
    assert evidence["target_bpm_version"] == "0.9.1"
    assert evidence["status"] == "accepted"
    assert evidence["result"] == "pass"
    assert evidence["target"]["identity"] == {"ID": "ubuntu", "VERSION_ID": "26.04"}
    ...
    assert snapshot["accepted_ref"] == evidence["attempts"][-1]["bpm_ref"]
...
def test_false_pass_and_completion_guard_diagnostics_cannot_be_accepted() -> None:
    attempts = _json(MANIFEST)["attempts"]

    assert [attempt["attempt"] for attempt in attempts] == [1, 2, 3]
    assert len({attempt["container_id"] for attempt in attempts}) == 3
    assert [attempt["raw_harness_result"] for attempt in attempts] == [
        "pass",
        "blocked",
        "pass",
    ]
    assert [attempt["accepted_disposition"] for attempt in attempts] == [
        "rejected-diagnostic-false-pass",
        "retained-diagnostic-completion-guard-catch",
        "accepted-clean-source-install-pass",
    ]
    assert [attempt["completion_marker_present"] for attempt in attempts] == [
        False,
        False,
        True,
    ]
    assert all(attempt["installed_state_reused"] is False for attempt in attempts)
    assert all(attempt["retained"] is True for attempt in attempts)
    assert all(attempt["container_state"] == "exited" for attempt in attempts)


def test_accepted_attempt_covers_every_documented_stage_and_runtime_boundary() -> None:
    evidence = _json(MANIFEST)
    attempt = evidence["attempts"][-1]
    accepted = evidence["accepted_attempt"]

    assert attempt["command_start_events"] == attempt["command_end_events"] == 41
    assert attempt["documented_commands_completed"] == 35
    ...
    assert accepted["python_version"].startswith("3.14.")
...
def test_transcript_hashes_and_isolation_are_complete() -> None:
    evidence = _json(MANIFEST)
    ...
    assert evidence["golden_image"]["local_exact_digest_reused"] is True
    assert evidence["golden_image"]["registry_pull_performed"] is False
...
def test_inventory_retains_stopped_resources_within_the_current_storage_contract() -> None:
    ...
    assert len(inventory["containers"]) == retention["container_count"] == 7
    assert len({container["id"] for container in inventory["containers"]}) == 7
