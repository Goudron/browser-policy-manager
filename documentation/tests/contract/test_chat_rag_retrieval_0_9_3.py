...
def test_retrieval_contract_pins_the_active_e5_exact_generation() -> None:
    ...
    assert (
        generation["sha256"] == hashlib.sha256((ROOT / generation["path"]).read_bytes()).hexdigest()
    )
    ...
    assert contract["selected_embedding"] == {
        ...
        "dimension": 768,
        "query_requirement": (
            "The caller supplies one finite L2-normalized E5 query vector. Encoding, model loading, and answer generation are outside this task."
        ),
    }
...
def test_retrieval_contract_is_same_locale_citable_and_independent_from_m4() -> None:
    ...
    assert (
        contract["entry_boundary"]["external_evidence"] == "disabled and absent from this retriever"
    )
    ...
    assert set(evidence["required_chunk_fields"]) >= {
        "bpm_version",
...
def test_retrieval_contract_fails_closed_on_unverified_or_stale_artifacts() -> None:
    ...
    assert (
        "matrix byte count/hash/shape/finiteness/L2 normalization"
        in integrity["validate_before_scan"]
    )
    assert (
        "metadata byte hash/shape/field set/canonical chunk order"
        in integrity["validate_before_scan"]
    )
