...
def test_admitted_runtime_is_immutable_local_and_not_a_public_model_service() -> None:
    ...
    assert runtime["artifact"]["sha256"] == "a50ee14f021a9d8e92e30f622f7e3be1318ee1125bb9a9ba8d2025388df48743"
    assert runtime["security_boundary"]["allowed_worker_transport"] == ["direct child-process pipe", "stdio"]
    assert set(runtime["security_boundary"]["forbidden"]) >= {
        "llama-server", "TCP listener", "HTTP listener", "wildcard bind", "LAN bind", "public model API",
...
def test_each_admitted_model_has_official_immutable_artifact_template_and_six_locale_gate() -> None:
    ...
    for model in config["admitted_models"]:
        ...
        assert model["template_and_reasoning"]["required_mode"] == "non-thinking (/no_think) for BPM answer benchmarking"
...
def test_gated_or_unofficial_gemma_and_conversions_are_not_silently_admitted() -> None:
    ...
    assert config["benchmark_admission"]["required_locales"] == ["en", "ru", "de", "zh-CN", "fr", "es-ES"]
