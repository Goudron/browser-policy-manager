...
def test_windows_10_outcome_is_an_accepted_unverified_boundary() -> None:
    evidence = _json(EVIDENCE)

    assert evidence["schema_version"] == 1
    assert evidence["backlog_item"] == "BPM091-M11-11"
...
def test_contract_and_feasibility_record_the_closed_windows_10_boundary() -> None:
    evidence = _json(EVIDENCE)
    contract = _json(CONTRACT)
    feasibility = FEASIBILITY.read_text(encoding="utf-8")
    outcome = contract["conditional_outcomes"]["Windows10"]
    ...
    assert contract["conditional_outcomes"]["Windows11"] == {
    ...
    assert evidence["disposition"]["next_backlog_item"] == "BPM091-M11-12"
    assert "Windows 10: unverified-no-actual-host-supplied" in feasibility
    assert "makes no Windows 10 validation claim" in feasibility
...
def test_boundary_record_did_not_start_or_modify_runtime_resources() -> None:
    ...
    assert evidence["verification"] == {
        "focused_contract": "pass-16",
        "full_documentation_contract": "pass-718-with-4-deselected",
        "docs_snapshot": "pass",
