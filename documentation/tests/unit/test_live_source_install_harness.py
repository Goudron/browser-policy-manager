...
def test_rendered_script_records_adapter_documented_runtime_and_shutdown_events() -> None:
    ...
    assert 'setsid bash -c "$command"' in script
    assert 'setsid bash -lc "$command"' not in script
...
def test_documented_run_requires_a_valid_explicit_backlog_owner() -> None:
    ...
    base = [
        ...
        "debian-13-5",
        "--run-id",
        "m11-05-debian-source-install",
        "--bpm-ref",
        "ff8271e577169c9063291087fb5f244b9fdb9139",
    ]

    with pytest.raises(SystemExit):
        parser.parse_args(base)
    with pytest.raises(SystemExit):
        parser.parse_args([*base, "--backlog-item", "M11-05"])

    args = parser.parse_args([*base, "--backlog-item", "BPM091-M11-05"])
    assert args.backlog_item == "BPM091-M11-05"
