...
def _settings_source_catalog(locale: str) -> dict[str, str]:
    return json.loads((I18N_SOURCE_DIR / locale / "settings.json").read_text(encoding="utf-8"))
    ...
    expected_mode_bodies = {
        ...
        "de": {
            "profiles.settings_mode_review_body": (
                "Beheben Sie zuerst Einträge, die Aufmerksamkeit brauchen."
            ),
        ...
        "fr": {
            "profiles.settings_mode_review_body": (
                "Corrigez d'abord les éléments qui demandent attention."
            ),
        ...
        "es-ES": {
            "profiles.settings_mode_review_body": (
                "Corrige primero los elementos que requieren atención."
            ),
