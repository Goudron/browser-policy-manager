...
LOCALE_UPDATE_RUNBOOK_PATH = REPO_ROOT / "docs" / "locale_update_runbook_2026-06-01.md"
LOCALE_OWNERSHIP_PATH = REPO_ROOT / "docs" / "locale_ownership_2026-06-01.md"
FIREFOX_SCHEMA_UPDATE_RUNBOOK_PATH = REPO_ROOT / "docs" / "firefox-schema-update-runbook.md"
...
def test_global_ui_locale_glossary_records_six_locale_terms():
    ...
    assert "| Term ID | English source term | ru | de | zh-CN | fr | es-ES | Notes |" in glossary
...
def test_global_ui_locale_glossary_consolidates_mozilla_qa_findings():
    ...
    term_rows = [row for row in mozilla_section.splitlines() if row.startswith("| mozilla.")]
...
def test_locale_ownership_records_single_maintainer_model():
    ...
    assert (
        "BPM is currently maintained by a single project maintainer, Valery Ledovskoy" in ownership
    )
    assert (
        "There is no separate translator team, localization vendor, or open community translation process"
        in ownership
    )
    assert (
    ...
    required_rows = _contract("locale_ownership_records_single_maintainer_model", "required_rows")
    for row in required_rows:
