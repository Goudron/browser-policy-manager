...
def _runtime_i18n_keys() -> set[str]:
    ...
    for path in _runtime_source_files():
        ...
        keys.update(re.findall(r"""key:\s*["'](profiles\.[A-Za-z0-9_.-]+)["']""", source))
        keys.update(
            re.findall(
...
def test_library_count_labels_are_locale_catalog_backed():
    ...
    library_source = (STATIC_DIR / "profiles_library_bootstrap.js").read_text(encoding="utf-8")
    catalogs = {locale: _read_locale(locale) for locale in ACTIVE_CATALOG_LOCALES}
...
def test_compare_locale_keys_are_source_and_runtime_catalog_backed():
    expected_compare_keys = {
        key
        for key in _read_source_namespace("en", "library")
        if key.startswith("profiles.compare_")
    }
    ...
    for locale in ACTIVE_CATALOG_LOCALES:
        ...
        source_compare_keys = {key for key in source_library if key.startswith("profiles.compare_")}
