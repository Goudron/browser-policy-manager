...
def test_profile_route_navigation_warns_when_editor_is_dirty():
    ...
    assert (
        'url.pathname === "/profiles" || url.pathname.startsWith("/profiles/")'
        in dirty_guard_source
    )
    ...
    assert (
        "if (event.metaKey || event.ctrlKey || event.shiftKey) return true;" in dirty_guard_source
    )
    assert (
...
def test_editor_mode_links_preserve_route_aware_returns_and_settings_focus():
    source = (REPO_ROOT / "app" / "static" / "profiles_workspace.js").read_text(encoding="utf-8")
    ...
    assert (
        '`/profiles/${currentId}/json${includeDeleted ? `?${includeDeletedSuffix}` : ""}`' in source
    )
    assert (
        '`/profiles/${currentId}/edit${includeDeleted ? `?${includeDeletedSuffix}` : ""}`' in source
    )
    assert (
        "const settingsFocusTarget = normalizeSettingsModeFocusTarget(routeFocusTarget);" in source
    )
...
def test_saved_guided_route_enables_settings_and_json_handoffs_after_first_save():
    ...
    assert response.status_code == 200
    assert (
        f'href="/profiles/{profile_id}/settings?return=/profiles/{profile_id}/edit"'
        in response.text
    )
...
def test_profile_settings_route_preserves_step8_json_handoff():
    ...
    assert response.status_code == 200
    assert f'href="/profiles/{profile_id}/json?focus=raw"' in response.text
...
def test_ai_step_locales_describe_esr_empty_state():
    ...
    assert ru_catalog["profiles.wizard_ai_esr_title"] == ("Эта схема не поддерживает настройки ИИ")
    assert ru_catalog["profiles.wizard_ai_esr_state"] == ("Эта схема не поддерживает настройки ИИ.")
