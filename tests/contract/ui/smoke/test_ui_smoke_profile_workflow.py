...
def test_release_ai_step_browser_regression_keeps_release_controls_available():
    ...
    export_response = client.get(f"/api/export/profiles/{created['id']}/firefox/policies.json")
...
def test_release_guided_ai_and_vpn_browser_regression_can_save_and_export():
    ...
    review_source = (root / "app" / "static" / "profiles_review.js").read_text(encoding="utf-8")
    ...
    assert (
        'ipProtectionManaged: typeof parsed?.IPProtectionAvailable === "boolean"' in review_source
    )
...
def test_create_corporate_cis_l2_browser_regression_can_download_policies_json():
    ...
    assert exported["policies"]["Preferences"]["media.peerconnection.enabled"]["Value"] is False
...
def test_library_to_editor_browser_regression_can_load_and_save_guided_homepage():
    ...
    assert (
        f"<title>{payload['name']} — Guided editor — Browser Policy Manager</title>"
...
def test_visual_and_json_same_profile_regression_can_save_without_conflict():
    ...
    assert (
        f"<title>{payload['name']} — JSON editor — Browser Policy Manager</title>"
...
def test_stale_save_conflict_browser_regression_does_not_overwrite_profile():
    ...
    root = Path(__file__).resolve().parents[4]
    command_deck = (root / "app" / "templates" / "profiles" / "_page_command_deck.html").read_text(
        encoding="utf-8"
    )
    workspace_source = (root / "app" / "static" / "profiles_workspace.js").read_text(
        encoding="utf-8"
    )
    data_source = (root / "app" / "static" / "profiles_data.js").read_text(encoding="utf-8")
