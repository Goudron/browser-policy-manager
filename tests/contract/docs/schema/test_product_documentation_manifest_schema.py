...
def _validate_semantics(manifest: dict[str, Any], target_map: dict[str, Any]) -> None:
    ...
    for topic_id, topic in topics.items():
        ...
        anchor_aliases = [
            alias for anchor in topic["anchors"].values() for alias in anchor["aliases"]
        ]
...
def _contracts() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    manifest = _indexed_json("architecture/examples/product-documentation-manifest-v1.example.json")
