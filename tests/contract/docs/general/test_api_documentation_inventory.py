...
def test_api_documentation_inventory_matches_openapi_operations():
    ...
    api_rows = [
        match.groupdict() for line in inventory.splitlines() if (match := API_ROW_RE.match(line))
    ]
    web_rows = [
        match.groupdict() for line in inventory.splitlines() if (match := WEB_ROW_RE.match(line))
    ]
