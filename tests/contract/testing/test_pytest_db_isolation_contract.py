...
def test_conftest_configures_worker_database_before_importing_app_db():
    ...
    app_db_import_index = source.index("from app.db import get_session")
...
def test_conftest_session_guard_disposes_and_cleans_worker_database():
    ...
    assert "_dispose_global_database_engine" not in source
    assert "cleanup_global_sqlite_engine" not in source
