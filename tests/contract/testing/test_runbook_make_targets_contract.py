...
def test_readme_keeps_product_audience_without_maintainer_start_commands():
    readme = _read("README.md")

    assert "Browser Policy Manager (BPM) is a FastAPI application" in readme
    assert "http://127.0.0.1:8000/help/" in readme
...
def test_make_dev_refreshes_local_documentation_before_starting_runtime():
    ...
    for target in (
        ...
        assert f"{target}: ai-extra-check" in makefile
...
def test_locale_runbook_uses_make_targets_for_repeatable_gates():
    ...
    stale_commands = {
        ".venv/bin/pytest -q tests/integration/locale/test_locale_catalogs.py",
        ".venv/bin/mypy app",
        ".venv/bin/ruff check .",
...
def test_epic_backlog_creation_runbook_defines_versioned_backlog_contract():
    ...
    required_phrases = {
        ...
        "updates every component for which that source offers a newer stable release",
        "Do not defer a newer stable component merely because its current floor still works",
