...
CONTRACT_PATH = (
    Path(__file__).resolve().parents[3] / "tests" / "fixtures" / "profile_compare_contract.json"
)
...
def test_profile_compare_contract_declares_two_saved_profile_model() -> None:
    ...
    assert contract["comparison_model"] == {
        ...
        "ordering": (
            "Policy rows sort by policy id before preference rows, which sort by preference name."
        ),
    ...
    for side in ("left", "right"):
        ...
        assert profile["name"]
        assert profile["schema_version"] == "release-153"
...
def test_profile_compare_rows_are_union_of_policies_and_preferences() -> None:
    ...
    expected_preference_names = sorted(
        _preference_names(left_flags) | _preference_names(right_flags)
    )
    expected_row_ids = [
