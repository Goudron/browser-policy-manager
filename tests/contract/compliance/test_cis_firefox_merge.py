...
def test_cis_merge_adds_missing_hardening_without_dropping_base_only_settings() -> None:
    base = build_wizard_starter_document("basic_corporate", "release-153")
    layer = build_cis_layer(2, "release-153")

    result = merge_base_with_cis_layer(
        base, layer, base_label="basic_corporate", cis_label="cis_l2"
    )
    ...
    validate_profile_policies_or_raise(
        ...
        load_policy_schema_for_channel("release-153"),
    )


def test_cis_merge_keeps_base_update_and_proxy_governance_for_review() -> None:
    base = build_wizard_starter_document("basic_corporate", "release-153")
    layer = build_cis_layer(1, "release-153")
...
def test_cis_merge_level_2_contains_level_1_plus_level_2_decisions() -> None:
    base = build_wizard_starter_document("blank", "release-153")
    l1 = merge_base_with_cis_layer(base, build_cis_layer(1, "release-153"))
    l2 = merge_base_with_cis_layer(base, build_cis_layer(2, "release-153"))
...
def test_cis_merge_keeps_soc_sanitize_cleanup_conflicts_for_review() -> None:
    base = build_wizard_starter_document("soc_hard", "release-153")
    layer = build_cis_layer(1, "release-153")
...
def test_cis_merge_preserves_classroom_kiosk_controls_and_adds_cis_hardening() -> None:
    base = build_wizard_starter_document("classroom_kiosk", "release-153")
    layer = build_cis_layer(2, "release-153")
