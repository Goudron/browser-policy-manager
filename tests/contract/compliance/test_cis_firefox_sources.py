...
def test_cis_source_helpers_handle_empty_ambiguous_and_missing_sources(tmp_path: Path) -> None:
    ...
    assert (
        find_benchmark("bench", upstream_version="1.0.0", base_dir=tmp_path)["upstream_version"]
        == "1.0.0"
    )
...
def test_cis_validator_rejects_invalid_policy_target_path(tmp_path: Path) -> None:
    ...
    assert not result.ok
    assert any(
        "policy target path must be a non-empty string list" in error for error in result.errors
    )
...
def test_cis_validator_reports_source_shape_and_empty_recommendations(tmp_path: Path) -> None:
    ...
    assert any(
        "missing required field 'public_versions_page_url'" in error for error in result.errors
    )
    assert any(
        "field 'benchmark_page_url' must be an HTTP(S) URL" in error for error in result.errors
    )
    assert any("file does not exist" in error for error in result.errors)
    assert any(
        "recommendation list is empty while official CIS PDF curation is pending" in warning
...
def test_cis_validator_reports_bad_recommendation_and_mapping_shapes(tmp_path: Path) -> None:
    ...
    assert any(
        "status 'manual' does not match recommendation mapping_status 'mapped'" in error
...
def test_cis_validator_reports_target_shape_errors(tmp_path: Path) -> None:
    ...
    assert any(
        "duplicate target ('policy', ('DisableTelemetry',))" in error for error in result.errors
    )
    assert any(
        "preference target path must contain exactly one preference key" in error
        for error in result.errors
    )
            ...
            "benchmarks": [
                {
                    ...
                    "by_level": {"L1": 1},
                    "schema_compatibility": {"release-153:valid": 1},
                }
            ],
            "warnings": [],
            "errors": [],
...
def test_cis_coverage_report_counts_statuses(tmp_path: Path) -> None:
    ...
    assert benchmark["schema_compatibility"] == {"esr-140.13:valid": 1, "release-153:valid": 1}
