...
def test_cis_level_2_includes_level_1_recommendations() -> None:
    l1 = build_cis_layer(1, "release-153")
    l2 = build_cis_layer(2, "release-153")
...
def test_cis_generated_layers_include_policy_and_preference_targets() -> None:
    layer = build_cis_layer(2, "release-153")
...
def test_write_generated_layers_creates_deterministic_json_documents(tmp_path) -> None:
    ...
    assert [path.name for path in written] == [
        ...
        "cis_l2.esr-153.0.json",
        "cis_l1.release-153.json",
        "cis_l2.release-153.json",
    ]
    document = json.loads((tmp_path / "cis_l2.release-153.json").read_text(encoding="utf-8"))
    assert document["level"] == 2
    assert document["schema_channel"] == "release-153"
...
def test_build_cis_layer_rejects_unknown_level_and_benchmark() -> None:
    ...
    with pytest.raises(ValueError, match="CIS level"):
        build_cis_layer(3, "release-153")

    with pytest.raises(ValueError, match="Unknown CIS benchmark"):
        build_cis_layer(1, "release-153", benchmark_id="missing")


def test_cis_generation_skips_unmapped_and_unsupported_targets(tmp_path) -> None:
    from tests.contract.compliance.test_cis_firefox_sources import (
        _mapping,
        _recommendation,
        _write_fixture,
    )
    ...
    unsupported["targets"][0]["schema_channels"] = {"release-153": "invalid"}
    _write_fixture(
        tmp_path,
        recommendations=[
            "ignored",
    ...
    layer = build_cis_layer(1, "release-153", base_dir=tmp_path)
...
def test_cis_generation_skips_recommendations_without_string_ids(tmp_path) -> None:
    from tests.contract.compliance.test_cis_firefox_sources import _mapping, _write_fixture
