...
def test_dev_readiness_distinguishes_disabled_missing_and_available_without_probe() -> None:
    ...
    assert all(
        readiness.provider_probe_performed is False
        for readiness in (disabled, missing, available)
    )
...
def test_dev_cli_json_is_content_free_and_make_dev_runs_the_check(monkeypatch, capsys) -> None:
    ...
    assert "dev: docs-install-dev ai-model-install-dev ai-runtime-install-dev ai-rag-install-dev ai-web-sources-check-dev" in makefile
...
def test_dev_cli_human_output_and_module_entrypoint_are_safe(monkeypatch, capsys) -> None:
    ...
    try:
        ...
        with pytest.raises(SystemExit) as exited:
            runpy.run_module("app.documentation.web_evidence_dev_config", run_name="__main__")
    finally:
        module.get_settings.cache_clear()
...
def test_readme_keeps_external_source_configuration_outside_the_0_9_3_release() -> None:
    ...
    assert "external-source research are" in readme
    assert "separate future work" in readme
