...
@pytest.mark.parametrize("channel", SUPPORTED_SCHEMA_CHANNELS)
def test_supported_channel_rejects_non_object_policy_document(channel: str):
    with pytest.raises(PolicyValidationError):
        validate_profile_policies_or_raise_for_channel(123, channel)  # type: ignore[arg-type]


@pytest.mark.parametrize("channel", ("release-153", "esr-153.0"))
def test_firefox_153_channels_accept_extension_allowed_permissions(channel: str):
    validate_profile_policies_or_raise_for_channel(
...
def test_release_schema_includes_newer_mozilla_policies():
    schema = _schema("release-153")
...
def test_preferences_value_type_is_controlled_by_declared_type():
    ...
    assert any(
        issue.path == ["Preferences", "browser.tabs.warnOnClose", "Value"] for issue in issues
    )
