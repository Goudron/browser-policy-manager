...
def test_validate_profile_policies_uses_json_schema_features():
    ...
    assert any(
        policy == "Proxy" and path == ("Proxy", "Mode") and "required property" in message
        for policy, path, message in summary
    )
    assert any(
        policy == "Mode"
...
def test_channel_validator_cache_recompiles_when_schema_artifact_changes(monkeypatch):
    validation.clear_policy_validator_cache()
    build_calls = []
    schema = {
        "type": "object",
    ...
    class _Validator:
        def iter_errors(self, _profile_policies):
            return []

    monkeypatch.setattr(validation, "load_policy_schema_for_channel", lambda _channel: schema)
    monkeypatch.setattr(
        validation,
        "_build_validator",
        lambda prepared: build_calls.append(prepared) or _Validator(),
    )

    validation.validate_profile_policies_for_channel({"DisableTelemetry": True}, "release-153")
    schema["properties"]["DisableTelemetry"]["type"] = "string"
    validation.validate_profile_policies_for_channel({"DisableTelemetry": "yes"}, "release-153")
...
def test_validate_profile_payload_with_schema_defaults_channel_and_validates(monkeypatch):
    ...
    monkeypatch.setattr(
        validation, "validate_profile_policies_or_raise_for_channel", _fake_validate
    )
