...
def _write_schema(path, *, channel: str = "release-153") -> None:
    payload = {
...
def _write_custom_schema(
    path, properties: dict[str, object], *, channel: str = "release-153"
) -> None:
    payload = {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "title": "Release Policies 149.0",
...
def test_get_schema_path_missing_file_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(FileNotFoundError):
        service._get_schema_path("release-153")


def test_load_policy_schema_and_get_definition(tmp_path, monkeypatch):
    release_file = tmp_path / "firefox-release-153.json"
    _write_schema(release_file)

    monkeypatch.setattr(service, "SCHEMAS_DIR", tmp_path)

    schema = service.load_policy_schema("release-153")
    definition = service.get_policy_definition("release-153", "DisableTelemetry")
    missing = service.get_policy_definition("release-153", "NoSuchPolicy")

    assert schema.channel == "release-153"
    ...
    cached = service.load_policy_schema("release-153")
...
def test_load_policy_schema_uses_safe_fallback_ui_metadata(tmp_path, monkeypatch):
    ...
    definition = service.load_policy_schema("release-153").get_policy("CustomPortal")
...
def test_real_policy_schemas_expose_ui_metadata_for_every_policy():
    section_ids = {section.id for section in service.load_policy_schema("release-153").ui_sections}
...
def test_json_schema_type_handles_oneof_non_dict_properties_items_and_fallback():
    ...
    assert (
        service._json_schema_type({"oneOf": [{"type": ""}, {"items": {"type": "string"}}]})
        == "array"
    )
