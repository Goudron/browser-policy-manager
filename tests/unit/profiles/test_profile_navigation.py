...
def test_profile_navigation_rejects_unsafe_return_and_focus_values():
    ...
    assert (
        profile_navigation.resolve_safe_profiles_return_url("/profiles?return=https://evil.test")
        is None
    )
    assert (
...
def test_profile_navigation_maps_settings_and_json_focus_targets():
    ...
    assert (
        profile_navigation.resolve_json_focus_target_from_settings_focus("policy:DisableTelemetry")
        == "policy:DisableTelemetry"
    )
    assert profile_navigation.resolve_json_focus_target_from_settings_focus("   ") is None
    assert profile_navigation.resolve_json_focus_target_from_settings_focus("custom-target") is None
    ...
    assert (
        profile_navigation.resolve_settings_focus_target_from_json_focus("deprecated:LegacyPolicy")
        == "settings-schema-shell-step-8"
    )
    assert (
        profile_navigation.resolve_settings_focus_target_from_json_focus("policy:DisableTelemetry")
        == "policy:DisableTelemetry"
    )
    assert (
...
def test_profile_navigation_builds_route_hrefs_with_context():
    ...
    assert profile_navigation.build_profile_route_path(8, "settings") == ("/profiles/8/settings")
    assert (
        profile_navigation.build_profile_route_path(
            8,
            "settings",
            include_deleted=True,
        )
