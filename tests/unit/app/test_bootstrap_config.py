...
def test_settings_version_matches_pyproject():
    ...
    assert pyproject["project"]["version"] == "0.9.5.1"
...
def test_project_version_falls_back_when_pyproject_cannot_be_read(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _distribution: (_ for _ in ()).throw(config_module.PackageNotFoundError),
    )
...
def test_db_module_uses_prefixed_database_settings(monkeypatch):
    ...
    config_module.get_settings.cache_clear()
    configured = config_module.get_settings()
    runtime = db_module.DatabaseRuntime(configured)

    assert runtime.database_url == "sqlite+aiosqlite:///./tmp-bootstrap.db"
    assert runtime.echo is True
