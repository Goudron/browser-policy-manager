    ...
    try:
        session.add_all(
            [
                ...
                Profile(name="current-release", schema_version="release-150", flags={"DisableTelemetry": True}),
            ]
        )
        session.commit()
        ...
        assert rows["legacy-esr-9"][0] == "esr-140.10"
        assert rows["legacy-release-current-minus-one"][0] == "release-150"
        assert rows["legacy-release-current-minus-one-copy"][0] == "release-150"
        assert rows["legacy-esr-8"][0] == LEGACY_ESR_140_8
        assert rows["legacy-release-older"][0] == LEGACY_RELEASE_148
        assert rows["current-release"][0] == "release-150"
...
def test_profile_schema_normalization_noops_when_no_legacy_channels_exist():
    ...
    try:
        session.add(
            Profile(name="current-only", schema_version="release-150", flags={"DisableTelemetry": True})
        )
        session.commit()
...
def test_app_startup_normalizes_selected_legacy_profile_channels():
    ...
    try:
        ...
        assert rows["startup-esr"] == "esr-140.10"
        assert rows["startup-release"] == "release-150"
