...
def test_schema_version_refs_and_invalid_key():
    assert SchemaVersion.from_key("firefox-esr14012") is SchemaVersion.ESR14012
    assert SchemaVersion.from_key("firefox-release152") is SchemaVersion.RELEASE152
    assert SchemaVersion.ESR14012.cache_subdir == "esr14012"
    assert "main" in SchemaVersion.RELEASE152.refs
...
def test_update_cache_returns_schema_meta(tmp_path):
    ...
    meta = manager.update_cache("release152")

    assert meta.version is SchemaVersion.RELEASE152
...
def test_load_force_refresh_falls_back_to_existing_cache_on_download_error(tmp_path, monkeypatch):
    ...
    assert manager.load("release152")["title"] == "cached"

    failing = SchemaManager(cache_dir=str(cache_dir), fetcher=lambda url, timeout: (503, b""))

    assert failing.load("release152", force_refresh=True)["title"] == "cached"
...
def test_download_and_cache_continues_after_fetch_exception(tmp_path, monkeypatch):
    ...
    meta = manager.update_cache("esr14012")
...
def test_normalize_schema_for_internal_use_adds_meta(monkeypatch):
    ...
    normalized = normalize_schema_for_internal_use(original, SchemaVersion.ESR14012)

    assert normalized["title"] == "Policies"
    assert normalized["$meta"]["bpm_source_version"] == "esr14012"
