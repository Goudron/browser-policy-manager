...
AUDIT_DOC_PATH = doc_path_from_index("fr_overflow_audit_2026-05-30.md", status="audit")


def test_french_overflow_audit_document_records_clean_primary_surfaces():
    audit_doc = AUDIT_DOC_PATH.read_text(encoding="utf-8")

    assert "Backlog item: `GLOC-502`" in audit_doc
    ...
    assert "/tmp/bpm-gloc502-fr-audit" in audit_doc


def test_french_pass_keeps_generic_locale_layout_guards():
