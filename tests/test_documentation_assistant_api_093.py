...
class _AssistantService:
    ...
    def status(self, *, locale: str, session_id: str) -> AssistantStatus:
        ...
        return AssistantStatus("ready", True, True, locale, "assistant_ready", "assistant_chat_send", "assistant_ready", 3)
    ...
    def events(self, *, request_id: str, session_id: str) -> tuple[ConversationStreamEvent, ...] | None:
        if request_id != "req_example" or session_id not in self.sessions:
            return None
        return (
            ConversationStreamEvent("accepted", request_id, "accepted", 1, "assistant_accepted", "assistant_chat_accepted", "assistant_chat_cancel"),
            ConversationStreamEvent("final", request_id, "answer", 2, "assistant_citations_validated", "assistant_chat_answer", "", "answer", "Grounded BPM answer.", ("src_example",)),
        )
    ...
    def source(self, *, request_id: str, source_id: str, session_id: str) -> AssistantSource | None:
        ...
        if source_id != "src_example":
            return None
        return AssistantSource(source_id, "BPM overview", "/help/en/user/overview.html", "en", "Approved BPM documentation excerpt.")
...
def test_same_origin_assistant_api_exposes_only_safe_status_chat_events_and_sources() -> None:
    ...
    with make_test_client(app) as client:
        ...
        rejected = client.post("/api/documentation-assistant/chat", json={"api_version": 1, "locale": "en", "question": "BPM"})
        ...
        assert unsupported_version.json()["detail"]["reason_code"] == "assistant_api_version_unsupported"
...
def test_external_claims_are_a_separate_optional_final_event_region() -> None:
    event = ConversationStreamEvent(
        ...
        ("src_local",),
        external_claims=(
            ConversationStreamExternalClaim("External Firefox note.", ("src_web",)),
        ),
    )
    ...
    assert _event_payload(event) == {
        ...
        "citations": ["src_local"],
        "external_claims": [
            {"text": "External Firefox note.", "citations": ["src_web"]}
        ],
    }
...
def test_unavailable_service_and_event_payload_keep_all_terminal_paths_safe() -> None:
    ...
    assert service.set_web_mode(
        locale="en", session_id="session", enabled=True, tab_id=TAB_ID
    ).enabled is False
...
def test_json_body_guard_handles_declared_and_streamed_invalid_payloads() -> None:
    cases = (
        ({"content-length": str(assistant_api.ASSISTANT_MAX_REQUEST_BYTES + 1)}, (b"{}",), "assistant_request_too_large"),
        ({"content-length": "not-a-number"}, (b"{}",), "assistant_invalid_request"),
        ({}, (b"x" * (assistant_api.ASSISTANT_MAX_REQUEST_BYTES + 1),), "assistant_request_too_large"),
