        ...
        "topics": {
            ...
            **{
                topic_id: {
                    ...
                    "anchors": anchors,
                    "output": {
                        locale: f"{locale}/{url_root}/{topic_id}.html"
                        for locale in locales
                    },
                }
...
def test_help_route_resolves_manifest_topics_aliases_and_unsupported_locale_fallback(
    ...
    assert topic_without_slash.headers["location"] == "/help/ru/user/ug-concept-choose-editor-surface/"
...
def test_help_route_preserves_accessibility_responsive_csp_and_theme_contract(
    ...
    for response in (docs_page, topic_page, missing_page):
        ...
        assert soup.find("meta", attrs={"name": "viewport"})["content"] == "width=device-width, initial-scale=1"
...
def test_documentation_manifest_validation_failures_return_no_catalog(
    ...
    assert_invalid_site(mutate_target_map=lambda target_map: target_map.update({"schema_version": 2}))
    assert_invalid_site(
    ...
    assert_invalid_site(mutate_target_map=lambda target_map: target_map.update({"bpm_version": "0.8.0"}))
    ...
    assert_invalid_site(lambda manifest: manifest["topics"]["user-guide"]["output"].update({"en": 42}))
...
def test_documentation_target_semantics_reject_malformed_records(tmp_path: Path) -> None:
    ...
    malformed_cases = [
        ...
        (
            {"topics": {}},
            {"targets": {"topic:user-guide": {"kind": "topic", "source_id": "other", "topic_id": "user-guide"}}},
        ),
        (
            {"topics": {}},
            {"targets": {"topic:user-guide": {"kind": "topic", "source_id": "user-guide", "topic_id": "missing"}}},
        ),
        (
            {"topics": {"user-guide": []}},
            {"targets": {"topic:user-guide": {"kind": "topic", "source_id": "user-guide", "topic_id": "user-guide"}}},
        ),
        (
...
def test_help_route_canonical_redirect_helpers_cover_alias_anchor_and_bad_topics(
    ...
    assert redirect.headers["location"] == "/help/en/user/ug-concept-choose-editor-surface/#a-anchored"

    assert docs_router._canonical_topic_redirect(locale="en", catalog=catalog, public_path="") is None
    ...
    assert docs_router._canonical_topic_redirect(
        locale="en",
