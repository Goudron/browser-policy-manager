...
def test_get_schema_or_404_loads_supported_profile(monkeypatch):
    ...
    monkeypatch.setattr(
        validation_module, "load_policy_schema_for_channel", lambda profile: sentinel
    )

    result = validation_module._get_schema_or_404("esr-140.13")
...
def test_get_schema_or_404_reports_unavailable_schema(monkeypatch):
    ...
    with pytest.raises(HTTPException) as excinfo:
        validation_module._get_schema_or_404("esr-140.13")
...
async def test_validate_profile_422_with_empty_issues(monkeypatch):
    ...
    monkeypatch.setattr(
        validation_module, "validate_profile_policies_or_raise", _raise_validation_error
    )
...
async def test_validate_profile_accepts_firefox_policies_document(monkeypatch):
    ...
    assert result == {"ok": True, "profile": "release-153"}
    assert seen == {
        "document": {"policies": {"DisableTelemetry": True}},
        "profile": "release-153",
    }
...
async def test_validate_profile_reports_firefox_document_shape_errors(monkeypatch):
    ...
    monkeypatch.setattr(
        validation_module, "validate_firefox_policies_document", _raise_import_error
    )
...
async def test_validate_profile_reports_firefox_document_policy_errors(monkeypatch):
    ...
    monkeypatch.setattr(
        validation_module, "validate_firefox_policies_document", _raise_validation_error
    )
