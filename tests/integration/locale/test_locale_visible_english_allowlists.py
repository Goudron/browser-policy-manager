...
ALLOWLIST_DOC_PATH = REPO_ROOT / "docs" / "locale_visible_english_allowlists_2026-05-30.md"
PLACEHOLDER_RULES_PATH = REPO_ROOT / "docs" / "locale_placeholder_identifier_rules_2026-05-29.md"
...
COMMON_UNTRANSLATED_PROSE_FRAGMENTS = tuple(CONTRACT["common_untranslated_prose_fragments"])
LOCALE_ALLOWED_EXAMPLES = {
    locale: tuple(examples) for locale, examples in CONTRACT["locale_allowed_examples"].items()
}
LOCALE_FORBIDDEN_FRAGMENTS = {
    locale: tuple(fragments) for locale, fragments in CONTRACT["locale_forbidden_fragments"].items()
}
AUDITED_KEYS = {locale: tuple(keys) for locale, keys in CONTRACT["audited_keys"].items()}
...
def _english_source_phrases(value: str) -> set[str]:
    ...
    for size in (4, 3, 2):
        for index in range(len(words) - size + 1):
            ...
            if any(
                allowed in phrase or phrase in allowed for allowed in ALLOWED_SHARED_ENGLISH_PHRASES
            ):
                continue
            phrases.add(phrase)
    return phrases
...
def _all_english_source_phrases(value: str) -> set[str]:
    ...
    for size in (5, 4, 3, 2):
        for index in range(len(source_words) - size + 1):
            ...
            if any(
                allowed.lower() in phrase.lower() or phrase.lower() in allowed.lower()
    ...
    expected = {
        "ru": {
            ...
            "profiles.compare_table_empty": (
                "Выберите профиль A и профиль B, чтобы сравнить их настройки."
            ),
        ...
        "de": {
            ...
            "profiles.compare_table_empty": (
                "Wählen Sie Profil A und Profil B aus, um ihre Einstellungen zu vergleichen."
            ),
            "profiles.compare_table_no_settings": (
                "Keines der Profile hat Einstellungen zum Vergleichen."
            ),
        ...
        "fr": {
            ...
            "profiles.compare_table_empty": (
                "Choisissez le profil A et le profil B pour comparer leurs paramètres."
            ),
            "profiles.compare_table_no_settings": (
                "Aucun des deux profils ne contient de paramètres à comparer."
            ),
        ...
        "es-ES": {
            ...
            "profiles.compare_table_empty": (
                "Elija el perfil A y el perfil B para comparar sus ajustes."
            ),
            "profiles.compare_table_no_settings": (
                "Ninguno de los perfiles tiene ajustes para comparar."
            ),
...
def test_corporate_cis_l2_visible_locale_strings_do_not_reuse_english_source_phrases():
    ...
    source_phrases = {
        key: _english_source_phrases(english_catalog[key]) for key in CORPORATE_CIS_L2_VISIBLE_KEYS
    }
...
def test_non_english_catalogs_do_not_reuse_english_source_phrases():
    english_catalog = _load_catalog("en")
    source_phrases = {
        key: _all_english_source_phrases(value) for key, value in english_catalog.items()
    }
        ...
        unexpected = sorted(
            {
                ...
                if word.lower() not in allowed_terms and not re.fullmatch(r"[A-Z]{2,}", word)
            }
        )
        if unexpected:
