...
def _visible_latin_tokens(locale: dict[str, str]) -> set[str]:
    locale_text = "\n".join(value for value in locale.values() if isinstance(value, str))
    ...
    return set(re.findall(r"[A-Za-z][A-Za-z0-9]*(?:[-/][A-Za-z0-9]+)*", scrubbed_text))
...
def test_ru_locale_visible_latin_tokens_stay_on_allowlist():
    ...
    unexpected_tokens = sorted(visible_tokens - allowed_by_shape - ALLOWED_VISIBLE_LATIN_TOKENS)
