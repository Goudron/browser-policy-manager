...
def test_locale_loading_assumptions_audit_records_key_decisions():
    ...
    required_fragments = (
        ...
        "`tests/integration/locale/test_ru_locale_quality.py`",
        "`README.md`",
        "`GLOC-003`",
        "`GLOC-004`",
        "`GLOC-401`",
...
def test_client_locale_resolver_uses_matrix_backed_rules_and_enabled_modes():
    platform_source = (REPO_ROOT / "app" / "static" / "profiles_platform.js").read_text(
        encoding="utf-8"
    )
    runtime_source = (REPO_ROOT / "app" / "static" / "profiles_runtime.js").read_text(
        encoding="utf-8"
    )
    library_source = (REPO_ROOT / "app" / "static" / "profiles_library_bootstrap.js").read_text(
        encoding="utf-8"
    )
