...
def test_linux_distribution_selection_names_exact_five_targets() -> None:
    ...
    selected_rows = [
        line for line in selection.splitlines() if line.startswith("| ") and " | `" in line
    ]
...
def test_linux_distribution_selection_freezes_exact_command_authoring_targets() -> None:
    ...
    assert (
        "newer release appearing after 2026-07-11 does not silently change this table" in selection
    )
