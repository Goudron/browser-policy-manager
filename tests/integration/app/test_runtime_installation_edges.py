...
def test_runtime_copy_metadata_removal_progress_and_cli_paths(
    ...
    monkeypatch.setattr(
        runtime,
        "_regular_non_symlink",
        lambda path: False if path == unsafe else original_regular(path),
    )
    ...
    assert '"verified": true' in runtime._result(
        runtime.RuntimeVerification("installed", True, "verified", "id")
    )
...
def test_runtime_extraction_and_verification_all_failure_forms(
    ...
    monkeypatch.setattr(
        runtime.tarfile, "open", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("bad"))
    )
    ...
    monkeypatch.setattr(
        runtime.tarfile,
        "open",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(tarfile.TarError("bad")),
    )
    with pytest.raises(runtime.RuntimeVerificationError, match="bundle_extraction_failed"):
...
def test_runtime_install_verify_remove_and_cli_remaining_failures(
    ...
    with pytest.raises(
        runtime.RuntimeVerificationError, match="refusing_to_remove_runtime_symlink"
    ):
