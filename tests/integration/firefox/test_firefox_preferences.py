...
def test_wizard_settings_catalog_is_single_source_of_truth():
    ...
    assert (
        catalog["sections_by_id"]["general"]["ui_maps"]["policy"][0]["label_key"]
        == "profiles.wizard_settings_general_startup"
    )
    assert (
        catalog["sections_by_id"]["general"]["ui_controls"]["policy"][0]["target"]
        == "field:wizard-homepage-start-page"
    )
    assert (
        catalog["sections_by_id"]["privacy"]["ui_maps"]["lockdown"][0]["fallback"]
        == "HTTPS-Only Mode"
    )
    assert (
        catalog["sections_by_id"]["privacy"]["preferences"]["controls"][0]["target"]
        == "preference-preset:privacy_tracking"
    )
...
def test_wizard_preferences_catalog_matches_gui_sections():
    ...
    assert [
        option["value"] for option in known_by_pref["browser.startup.page"]["value_options"]
    ] == [0, 1, 3]
    ...
    assert [
        option["value"] for option in known_by_pref["browser.download.folderList"]["value_options"]
    ] == [0, 1, 2]
    assert known_by_pref["media.autoplay.default"]["value_control"] == "select"
    assert [
        option["value"] for option in known_by_pref["media.autoplay.default"]["value_options"]
    ] == [0, 1, 5]
    assert (
        known_by_pref["browser.newtabpage.activity-stream.topSitesRows"]["value_control"]
        == "select"
    )
    assert [
        option["value"]
        for option in known_by_pref["browser.newtabpage.activity-stream.topSitesRows"][
            "value_options"
        ]
    ] == [1, 2, 3, 4]
    ...
    assert [
        option["value"]
        for option in known_by_pref["browser.contentblocking.category"]["value_options"]
    ] == ["standard", "strict", "custom"]
    assert known_by_pref["network.cookie.cookieBehavior"]["value_control"] == "select"
    assert [
        option["value"]
        for option in known_by_pref["network.cookie.cookieBehavior"]["value_options"]
    ] == [0, 1, 4, 5]
    assert known_by_pref["network.trr.mode"]["value_control"] == "select"
    assert [option["value"] for option in known_by_pref["network.trr.mode"]["value_options"]] == [
        0,
        2,
        3,
        5,
    ...
    assert [
        option["value"] for option in known_by_pref["permissions.default.geo"]["value_options"]
    ] == [0, 1, 2]
    assert known_by_pref["permissions.default.camera"]["value_control"] == "select"
    assert [
        option["value"] for option in known_by_pref["permissions.default.camera"]["value_options"]
    ] == [0, 1, 2]
    assert known_by_pref["permissions.default.microphone"]["value_control"] == "select"
    assert [
        option["value"]
        for option in known_by_pref["permissions.default.microphone"]["value_options"]
    ] == [0, 1, 2]
    assert known_by_pref["permissions.default.desktop-notification"]["value_control"] == "select"
    assert [
        option["value"]
        for option in known_by_pref["permissions.default.desktop-notification"]["value_options"]
    ] == [0, 1, 2]
    ...
    assert (
        len(
            next(
                bundle
                for bundle in sections["privacy"]["bundles"]
                if bundle["id"] == "privacy_site_permissions_lockdown"
            )["items"]
        )
        == 4
    )
    assert [
        ...
        for item in next(
            bundle
            for bundle in sections["privacy"]["bundles"]
            if bundle["id"] == "privacy_doh_hardened"
        )["items"]
    ] == [
        "network.trr.mode",
