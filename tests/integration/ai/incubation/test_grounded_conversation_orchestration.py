...
def test_orchestrator_rewrites_a_private_excessive_quote_without_reusing_evidence_context() -> None:
        ...
        json.dumps(
            {
                ...
                "sections": [
                    {
                        "text": (
                            "Use the documented workflow, then validate the profile before deployment."
                        ),
    ...
    assert outcome.disposition == "answer"
    assert (
        outcome.text == "Use the documented workflow, then validate the profile before deployment."
    )
...
def test_relevance_gate_removes_higher_scoring_debian_before_the_worker_receives_evidence() -> None:
    ...
    outcome = orchestrator.ask(ConversationRequest("en", "How do I install BPM on Ubuntu 26.04?"))
...
def test_capability_questions_use_reviewed_six_locale_copy_without_downstream_work() -> None:
    ...
    for locale, question in questions.items():
        ...
        def scope_gate(_: ConversationRequest, actual_calls: list[str] = calls) -> ScopeDecision:
...
def test_enabled_web_path_merges_lower_priority_evidence_and_separates_claims() -> None:
    ...
    orchestrator = GroundedConversationOrchestrator(
        ...
        evidence_merger=ConservativeEvidenceMerger(today=lambda: date(2026, 7, 30)),
...
def test_web_provider_failure_preserves_the_local_answer_pipeline() -> None:
    ...
    outcome = orchestrator.ask(ConversationRequest("en", "BPM question", web_mode="request_web"))
...
def test_orchestrator_stops_at_every_pre_generation_guard_and_reports_lifecycle() -> None:
    ...
    for decision, expected in (
        ...
        assert (
            orchestrator.ask(ConversationRequest("en", "ordinary BPM question")).reason_code
            == expected
        )
        assert retriever.locales == [] and worker.requests == []
...
def test_orchestrator_converts_retrieval_and_packing_failures_to_safe_abstention() -> None:
    ...
    assert (
        orchestrator.ask(ConversationRequest("en", "How do Firefox policies work?")).reason_code
        == "assistant_evidence_unavailable"
    )
    assert worker.requests == []
    ...
    class EmptyRelevance:
        def filter(
            self, request: ConversationRequest, result: RetrievalResult
        ) -> RelevanceDecision:
            del request
    ...
    assert (
        orchestrator.ask(ConversationRequest("en", "How do Firefox policies work?")).reason_code
        == "assistant_relevance_no_matching_evidence"
    )
    assert worker.requests == []
...
def test_orchestrator_validates_all_request_and_generation_shapes() -> None:
    ...
    for parsed, expected in (
        ...
        (
            ParsedGeneration("answer", "answer", ("topic:alpha", "topic:alpha")),
            "assistant_invalid_citations",
        ),
    ):
        assert (
            GroundedConversationOrchestrator._validated_outcome(
                parsed, valid_evidence, "en"
            ).reason_code
            == expected
        )
...
def test_web_generation_and_merged_output_fail_closed_at_each_boundary() -> None:
    ...
    assert (
        local._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    def with_web(
        web_result: object, merged: object | Exception
    ) -> GroundedConversationOrchestrator:
        ...
        orchestrator._merged_generation_validator = SimpleNamespace(
            validate=lambda *_: SimpleNamespace(disposition="abstain")
        )  # type: ignore[assignment]
        orchestrator._merged_generation_payload_builder = lambda value: value  # type: ignore[assignment]
        return orchestrator

    cancelled_web = with_web(
        SimpleNamespace(state="local_only", reason_code="assistant_cancelled"), SimpleNamespace()
    )
    assert (
        cancelled_web._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    fallback = with_web(
        SimpleNamespace(state="ready", lease=Lease(object())), RuntimeError("merge")
    )
    assert (
        fallback._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    abstaining = with_web(
        SimpleNamespace(state="ready", lease=Lease(object())),
        SimpleNamespace(state="abstain", reason_code="conflict"),
    )
    assert (
        abstaining._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    incomplete = with_web(
        SimpleNamespace(state="ready", lease=Lease(object())),
        SimpleNamespace(state="local_only", lease=closed),
    )
    assert (
        incomplete._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    for answer, expected in (
        ...
        (
            SimpleNamespace(disposition="answer", local_text="", local_citations=()),
        ...
        (
            SimpleNamespace(
                ...
                local_text="local",
                local_citations=(citation,),
                external_claims=(SimpleNamespace(citation_ids=("missing",)),),
                external_citations=(),
            ),
            "assistant_external_claim_citation_invalid",
        ),
    ):
        assert (
            GroundedConversationOrchestrator._merged_outcome(answer, "en").reason_code == expected
        )
...
def test_orchestrator_context_and_evidence_terminal_paths_are_bounded() -> None:
    ...
    assert (
        clarifying.ask(ConversationRequest("en", "How do Firefox policies work?")).reason_code
        == "need_evidence"
    )
    assert worker.requests == []
    ...
    assert (
        ...
        == "assistant_invalid_context_update"
    )
...
def test_local_rewrite_cancellation_and_request_size_boundaries() -> None:
    ...
    assert (
        GroundedConversationOrchestrator._validate_request(
...
def test_web_generation_cancellation_retrieval_and_validation_failures_fall_closed() -> None:
    ...
    def assembled(
        web_result: object | Exception, validator: Validator | None = None
    ) -> GroundedConversationOrchestrator:
        orchestrator, _, _, _ = _orchestrator()
    ...
    assert (
        retrieval_failure._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    assert (
        no_lease._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    assert (
        merged_without_lease._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    assert (
        before_generation._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    checks = iter((False, True))
    assert (
        after_generation._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
    ...
    invalid = assembled(
        SimpleNamespace(state="ready", lease=Lease(object())), Validator(RuntimeError("bad"))
    )
    assert (
        invalid._generate_with_optional_web(
            request,
            evidence=evidence,
            dialogue=(),
            lifecycle_callback=None,
