...
def _authorization(
    *, question: str = "How do I configure Firefox policies in BPM?", locale: str = "en"
    ...
    WebEvidenceConfiguration, WebEvidenceConsentStore, ConversationRequest, WebEvidenceAuthorization
]:
...
def test_one_consented_request_uses_only_fixed_post_and_returns_a_temporary_external_lease() -> (
    None
):
    ...
    with result.lease as evidence:
        ...
        assert (
            evidence[0].citation.source_url
        ...
        assert evidence[0].snippets == (
            "Firefox Enterprise Policies define managed browser settings.",
        )
...
def test_rejected_url_or_injected_content_is_inert_and_returns_local_only(
    ...
    transport = _Transport(
        ProviderHttpResponse(
            ...
            "application/json",
            "identity",
            _provider_body(url=url, title=title, snippets=snippets),
        )
    )
    result = _retriever(transport, configuration=configuration, store=store).retrieve(
        request, authorization=authorization
...
def test_response_limits_failure_and_one_attempt_never_retry_or_fetch_a_result_url() -> None:
    configuration, store, request, authorization = _authorization()
    transport = _Transport(ProviderHttpResponse(302, "application/json", "identity", b"{}"))
    result = _retriever(transport, configuration=configuration, store=store).retrieve(
        request, authorization=authorization
    )
    ...
    assert (
        _retriever(transport, configuration=configuration, store=store)
        .retrieve(request, authorization=authorization)
        .reason_code
    ...
    oversized = _Transport(
        ProviderHttpResponse(
            200, "application/json", "identity", b"x" * (MAX_PROVIDER_RESPONSE_BYTES + 1)
        )
    )
    assert (
        _retriever(oversized, configuration=configuration, store=store)
        .retrieve(request, authorization=authorization)
        .reason_code
