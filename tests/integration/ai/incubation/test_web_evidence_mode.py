...
def test_release_web_mode_is_off_by_default_and_retained_until_explicitly_disabled() -> None:
    store = WebEvidenceModeStore(configuration=WebEvidenceConfiguration(True, "server-only-token"))
    ...
    enabled = store.set_enabled(session_id="browser-session", locale="ru", enabled=True)
    ...
    unchanged = store.set_enabled(session_id="browser-session", locale="ru", enabled=True)
    assert unchanged.state_epoch == 1

    disabled = store.set_enabled(session_id="browser-session", locale="ru", enabled=False)
...
def test_release_web_mode_cannot_enable_without_administrator_configuration() -> None:
    ...
    missing_token = WebEvidenceModeStore(configuration=WebEvidenceConfiguration(True))

    assert (
        disabled.set_enabled(session_id="browser-session", locale="en", enabled=True).reason_code
        == "assistant_web_disabled"
    )
    unavailable = missing_token.set_enabled(session_id="browser-session", locale="en", enabled=True)
