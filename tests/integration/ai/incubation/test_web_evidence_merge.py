...
def test_merge_keeps_local_context_first_and_requires_a_separate_external_claim_with_citation() -> (
    None
):
...
def test_stale_or_undated_web_evidence_falls_back_to_local_only(
...
def test_non_answer_cannot_smuggle_local_or_external_text_and_closed_lease_cannot_be_reopened() -> (
    None
):
    merge = ConservativeEvidenceMerger(today=lambda: date(2026, 7, 30)).merge(
        local=_local(), external=(_external(),)
...
def test_merger_returns_local_only_for_empty_oversized_or_unpackable_external_context() -> None:
    ...
    unpackable = merger.merge(local=_local(), external=(_external(snippets=("safe " * 2_000,)),))
    ...
    assert (
        ...
        == unpackable.reason_code
        == oversized_local.reason_code
        == ("assistant_web_evidence_over_budget")
    )
    assert all(
        result.state == "local_only"
        for result in (no_external, too_many, unpackable, oversized_local)
    )
...
def test_merged_validator_rejects_invalid_context_payloads_sections_and_claim_shapes() -> None:
    ...
    assert (
        merge_module.MergedGenerationResponseValidator._parse(
            "x" * (merge_module.MAX_MODEL_RESPONSE_BYTES + 1)
        )
        is None
    )
    assert (
    ...
    assert (
        validator._terminal({"local_sections": [], "external_claims": []}, "answer").reason_code
        == "assistant_merged_output_invalid"
    )
    assert (
        validator._terminal({"local_sections": [], "external_claims": []}, "clarify").reason_code
        == "assistant_merged_clarify"
    )
    assert (
        validator._answer({"local_sections": {}, "external_claims": []}, view).reason_code
        == "assistant_merged_output_invalid"
    )
    assert (
        validator._answer({"local_sections": [], "external_claims": []}, view).reason_code
...
def test_merged_validator_and_helpers_fail_closed_for_invalid_local_sections_dates_and_ids() -> (
    None
):
    local = _local()
    ...
    joined_too_long = [
        {"text": "x" * 1_000, "citation_ids": [citation.citation_id]} for _ in range(5)
    ]
    ...
    assert (
        merge_module._resolve_ids((citation.citation_id, citation.citation_id), local.citations)
        is None
    )
