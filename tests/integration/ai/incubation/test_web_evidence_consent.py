...
def test_default_and_missing_byok_are_normal_local_only_states() -> None:
    ...
    assert disabled.disclose(
        session_id="session", locale="en", question="BPM question"
    ).reason_code == ("assistant_web_disabled")
    assert (
        missing_credential.disclose(
            session_id="session", locale="en", question="BPM question"
        ).reason_code
        == "assistant_web_credential_unavailable"
    )
    assert (
        WebEvidenceConfiguration.from_settings(Settings()).availability_code
...
def test_disclosure_shows_exact_query_and_required_brave_privacy_facts_without_the_key() -> None:
    ...
    assert "test-subscription-token" not in repr(
        WebEvidenceConfiguration(True, "test-subscription-token")
    )
...
def test_exact_question_session_locale_and_one_time_approval_are_required() -> None:
    ...
    next_disclosure = store.disclose(
        session_id="session-a", locale="en", question="BPM Firefox policy"
    )
    assert next_disclosure.disclosure is not None
    next_approval = store.approve(
        session_id="session-a", consent_id=next_disclosure.disclosure.consent_id
    )
...
def test_expiry_clear_and_question_bounds_remove_any_pending_capability() -> None:
    ...
    assert store.approve(
        session_id="session", consent_id=disclosed.disclosure.consent_id
    ...
    assert store.approve(
        session_id="session", consent_id=second.disclosure.consent_id
    ).reason_code == ("assistant_web_consent_unavailable")
    assert (
        store.disclose(
            session_id="session", locale="en", question="x" * (WEB_QUERY_CHARACTERS_MAX + 1)
        ).reason_code
        == "assistant_web_consent_invalid"
    )
    assert (
        store.disclose(
            session_id="session",
            locale="en",
            question=" ".join("word" for _ in range(WEB_QUERY_WORDS_MAX + 1)),
        ).reason_code
        == "assistant_web_consent_invalid"
    )
