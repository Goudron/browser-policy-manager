...
def _authorized() -> tuple[
    ...
    disclosed = store.disclose(session_id="browser-session", locale="en", question=request.question)
...
def test_retriever_shutdown_clears_owned_rate_and_consent_state() -> None:
    configuration = WebEvidenceConfiguration(True, "server-only-token")
    limiter = WebEvidenceRateLimiter()
    consent_store = _ClearStore()
    ...
    assert limiter.reserve("session") is True

    retriever.shutdown()

    assert consent_store.clear_calls == 1
    assert limiter.reserve("session") is True

    no_consent_limiter = WebEvidenceRateLimiter()
    ScopedWebEvidenceRetriever(
        configuration=configuration,
        mode_store=WebEvidenceModeStore(configuration=configuration),
        scope_gate=lambda _: ScopeDecision("allow", "scope_allowed"),
        transport=_Transport(_response(_provider_payload())),
        rate_limiter=no_consent_limiter,
    ).shutdown()
...
def test_http_transport_and_bounded_body_reject_redirect_type_and_transport_errors() -> None:
    ...
    class _Client:
        def __init__(
            self, response: _Response | None = None, failure: Exception | None = None
        ) -> None:
...
def test_retriever_short_circuits_all_pre_network_boundaries_and_faulty_cancellation() -> None:
    ...
    assert (
        ...
        .reason_code
        == "assistant_web_scope_unavailable"
    )
    assert (
        retriever.retrieve(
    ...
    assert (
        _retriever(configuration=WebEvidenceConfiguration(), store=store, transport=transport)
        .retrieve(request, authorization=authorization)
        .reason_code
...
def test_retriever_handles_cancellation_consumption_failure_and_empty_provider_evidence() -> None:
    ...
    assert (
        ...
        .retrieve(request, authorization=authorization, cancellation_check=cancel_after_scope)
        .reason_code
        == "assistant_cancelled"
    )
    ...
    assert (
        _retriever(configuration=configuration, store=store, transport=transport)
        .retrieve(request, authorization=authorization, cancellation_check=cancel_after_reservation)
        .reason_code
        == "assistant_cancelled"
    )
    assert transport.calls == []
    ...
    assert (
        _retriever(configuration=configuration, store=false_store, transport=failed_transport)
        .retrieve(
    ...
    assert (
        _retriever(configuration=configuration, store=store, transport=_Transport(empty))
        .retrieve(request, authorization=authorization)
        .reason_code
        == "assistant_web_no_evidence"
    )
...
def test_response_item_url_text_and_metadata_guards_reject_every_unsafe_shape() -> None:
    ...
    assert (
        web_evidence._approved_source_url("https://mozilla.github.io:bad/policy-templates/x")
        is None
    )
    assert (
        web_evidence._approved_source_url("https://mozilla.github.io/policy-templates/../x") is None
    )
