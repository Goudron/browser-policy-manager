...
def test_evidence_packer_strips_active_markup_and_enforces_item_and_packet_byte_caps() -> None:
    packer = EvidencePacker(
        lambda value: len(value.split()), bpm_version="0.9.3", maximum_context_tokens=200
    )
    active = _candidate(
...
def test_nonlocal_evidence_and_data_delimiters_cannot_be_promoted_to_worker_input(
    tmp_path: Path,
) -> None:
    ...
    payload = json.loads(
        worker._serialize_request(InferenceRequest("en", "question", '{"text":"safe"}'))
    )
