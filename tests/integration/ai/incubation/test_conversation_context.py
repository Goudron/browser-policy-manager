...
def test_same_locale_follow_up_uses_bounded_resolved_context_and_new_citation() -> None:
    ...
    first = orchestrator.ask(ConversationRequest("en", "ESR?", session_id=session_id))
    follow_up = orchestrator.ask(ConversationRequest("en", "ESR?", session_id=session_id))
...
def test_context_store_prunes_before_creation_and_never_exceeds_global_session_cap() -> None:
    now = 0.0
    store = ConversationContextStore(
        idle_ttl_seconds=5.0,
        absolute_ttl_seconds=100.0,
        max_sessions=2,
        clock=lambda: now,
    )
    first = store.create(locale="en", bpm_version="0.9.3")
    second = store.create(locale="en", bpm_version="0.9.3")
    assert store.contains(first) is True
    assert store.contains(second) is True
    with pytest.raises(ConversationContextUnavailable, match="assistant_session_limit"):
        store.create(locale="en", bpm_version="0.9.3")

    now = 5.0
    third = store.create(locale="en", bpm_version="0.9.3")
    assert store.contains(first) is False
    assert store.contains(second) is False
    assert store.contains(third) is True
...
def test_ninth_answer_evicts_only_the_oldest_of_eight_grounded_pairs() -> None:
    ...
    orchestrator = _orchestrator(
        ...
        retriever,
        worker,
        [ParsedGeneration("answer", f"answer-{number}", ("topic:alpha",)) for number in range(9)],
    )
...
def test_context_store_prunes_entities_for_a_disjoint_new_topic_and_expires_on_sweep() -> None:
    now = 0.0
    store = ConversationContextStore(
        idle_ttl_seconds=2, absolute_ttl_seconds=100, clock=lambda: now
    )
    session_id = store.create(locale="en", bpm_version="0.9.3")
    store.record(
        session_id=session_id,
