...
def test_invalid_or_failed_probe_fails_closed_to_safe_setup_diagnostics() -> None:
    ...
    assert (
        invalid.last_local_error_class
        == invalid_web.last_local_error_class
        == failing.last_local_error_class
        == "setup"
    )
    assert (
        invalid.assistant_ready is invalid_web.assistant_ready is failing.assistant_ready is False
    )
