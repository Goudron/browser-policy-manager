...
def test_download_redirect_validation_and_write_guards(
    ...
    response = httpx.Response(
        302,
        headers={"location": "https://cdn.hf.co/model"},
    ...
    for headers in ({}, {"location": "http://example.invalid/model"}):
        bad = httpx.Response(
            302, headers=headers, request=httpx.Request("GET", "https://huggingface.co/model")
        )
    ...
    with pytest.raises(installation.ArtifactVerificationError, match="exceeds"):
        installer._write_download(
            httpx.Response(200, content=b"x" * (small_artifact.byte_count + 1)), part, 0, None, None
        )
