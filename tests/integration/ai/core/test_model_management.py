...
def test_controller_status_starts_cancellation_and_visibility_are_session_bounded(
    ...
    assert controller.status(locale="en", session_id="two")["operation"] == {
        "state": "running",
        "busy": True,
    }
    ...
    assert (
        controller.cancel(session_id="one", operation_id=verify_operation.operation_id)["accepted"]
        is False
    )
    ...
    assert (
        controller.status(locale="en", session_id="one")["verification"]["state"] == "not-installed"
    )
...
def test_controller_failure_progress_and_snapshot_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    controller._operation = operation
    controller._record_progress(operation, SimpleNamespace(downloaded_bytes=-1, total_bytes=5))
    assert operation.downloaded_bytes == 0 and operation.total_bytes == 5
    controller._record_progress(operation, SimpleNamespace(downloaded_bytes=10, total_bytes=5))
    ...
    assert (
        controller._operation_payload(management._Operation("id", "s", "verify"))["cancellable"]
        is False
    )
...
def test_controller_from_settings_constructs_the_pinned_installer(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        management,
        "get_settings",
        lambda: SimpleNamespace(DATA_DIR=Path("/data"), AI_MODEL_DOWNLOAD_TIMEOUT_SECONDS=9),
    )
