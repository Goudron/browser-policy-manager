...
def test_local_citation_resolver_returns_only_valid_reviewed_chunk(tmp_path: Path) -> None:
    ...
    (generation / "chunks.json").write_text(
        json.dumps({"chunks": [{"topic_id": "overview", "anchor_id_or_root": "root", "heading_path": ["Overview"], "text": "Reviewed text"}]}),
        encoding="utf-8",
    )
...
def test_release_resolver_accepts_only_reviewed_citation_kinds(tmp_path: Path) -> None:
    ...
    trusted = ExternalWebCitation("web", "provider", "https://example.test", "Title", (), "now", locale="en")
    unknown_locale = ExternalWebCitation("web", "provider", "https://example.test", "Title", (), "now", locale="xx")
...
def test_local_citation_resolver_rejects_invalid_metadata_and_unmatched_chunks(tmp_path: Path) -> None:
    resolver = runtime.LocalCitationResolver(tmp_path)
    ...
    (generation / "chunks.json").write_text(
        json.dumps(
            {
                "chunks": [
                    {"topic_id": "overview", "anchor_id_or_root": "root", "heading_path": [], "text": "text"},
                    {"topic_id": "other", "anchor_id_or_root": "root", "heading_path": ["Other"], "text": "text"},
                ]
            }
        ),
        encoding="utf-8",
    ...
    (generation / "chunks.json").write_text(
        json.dumps(
            {"chunks": [{"topic_id": "overview", "anchor_id_or_root": "root", "heading_path": [""], "text": "text"}]}
        ),
        encoding="utf-8",
    )
    assert resolver(_citation()) is None
        ...
        json.dumps(
            {
                "chunks": [
                    {"topic_id": "other", "anchor_id_or_root": "root", "heading_path": ["Other"], "text": "text"},
                    {"topic_id": "overview", "anchor_id_or_root": "root", "heading_path": ["Overview"], "text": "text"},
                ]
            }
        ),
        encoding="utf-8",
