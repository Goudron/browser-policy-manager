...
class _Generation:
    ...
    def validate_generation(root: Path, config: dict) -> dict:
        return json.loads((root / config["matrix"]["root_manifest_file_name"]).read_text(encoding="utf-8"))
...
def _write_ready_state(root: Path, fingerprint: str = "fingerprint") -> None:
    ...
    pointer_path.write_text(
        json.dumps({"generation_id": generation_id, "manifest_sha256": manifest_sha256}), encoding="utf-8"
    )
...
def test_provision_retains_verified_generation_without_dita_extraction(
    ...
    monkeypatch.setattr(rag_bootstrap, "get_settings", lambda: SimpleNamespace(DATA_DIR=tmp_path, APP_VERSION="0.9.3"))
    ...
    assert result["cache_misses"] == {locale: 0 for locale in _Generation._read_json(Path())["input"]["locales"]}
...
def test_provision_adopts_a_verified_legacy_generation_without_extraction(
    ...
    monkeypatch.setattr(
        rag_bootstrap,
        "_extract_chunks",
        lambda destination: (_ for _ in ()).throw(AssertionError(f"unexpected extraction: {destination}")),
    )
...
def test_json_source_guards_and_chunks_validation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert rag_bootstrap._chunks_match_current_sources(
        {"chunks": [{"source_dita_path": "documentation/source.dita", "source_sha256": rag_bootstrap._sha256(source)}]}
    )
...
def test_installer_copies_pinned_source_and_preserves_invalid_existing_install(
    ...
    monkeypatch.setattr(rag_bootstrap, "MODEL_FILES", {"model.bin": hashlib.sha256(payload).hexdigest()})
    ...
    assert installer.provision() == root / "model"
    monkeypatch.setattr(rag_bootstrap, "verify_model_directory", lambda path: (_ for _ in ()).throw(ValueError()))
...
def test_provision_rebuilds_generation_and_rejects_wrong_chunk_version(
    ...
    monkeypatch.setattr(rag_bootstrap, "_published_source_fingerprint", lambda: "fingerprint")
    monkeypatch.setattr(rag_bootstrap, "_extract_chunks", lambda path: path.write_text('{"bpm_version":"0.9.3"}', encoding="utf-8"))
    assert rag_bootstrap.provision()["generation_id"] == "raggen-v1-new"
    monkeypatch.setattr(rag_bootstrap, "_extract_chunks", lambda path: path.write_text('{"bpm_version":"wrong"}', encoding="utf-8"))
    with pytest.raises(rag_bootstrap.RagBootstrapError, match="active BPM version"):
        rag_bootstrap.provision()


def test_download_checksum_and_cli_failure_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    class Response:
    ...
    rag_bootstrap.E5ModelInstaller._download(destination, "download.bin", hashlib.sha256(b"ok").hexdigest())
    ...
    monkeypatch.setattr(rag_bootstrap, "provision", lambda: (_ for _ in ()).throw(rag_bootstrap.RagBootstrapError("bad")))
...
def test_bootstrap_copy_download_import_and_chunk_edge_paths(
    ...
    installer = rag_bootstrap.E5ModelInstaller(tmp_path / "root", source_model_dir=tmp_path / "missing")
    monkeypatch.setattr(rag_bootstrap, "MODEL_FILES", {"model.bin": hashlib.sha256(b"payload").hexdigest()})
    monkeypatch.setattr(rag_bootstrap, "MODEL_DIRECTORY_NAME", "model")
    ...
    failing = rag_bootstrap.E5ModelInstaller(tmp_path / "failure", source_model_dir=tmp_path / "missing")
    monkeypatch.setattr(
        failing,
        "_download",
...
def test_bootstrap_fingerprints_active_generation_and_cli_ready_paths(
    ...
    assert rag_bootstrap._active_generation_result(
    ...
    manifest.write_text('{"generation_id":"wrong","source_manifest_sha256":"source-manifest"}', encoding="utf-8")
...
def test_bootstrap_remaining_copy_chunk_and_active_generation_guards(
    ...
    rag_bootstrap.E5ModelInstaller._download(WorkingDestination(), "working.bin", hashlib.sha256(b"").hexdigest())  # type: ignore[arg-type]
    ...
    for relative in (
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(relative, encoding="utf-8")
    monkeypatch.setitem(sys.modules, "build_docs", SimpleNamespace(_source_fingerprint=lambda: "docs"))
