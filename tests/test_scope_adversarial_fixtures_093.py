...
def test_adversarial_scope_fixtures_fail_closed_before_similarity_retrieval_or_worker() -> None:
    ...
    assert all(outcome.disposition == contract["acceptance"]["expected_disposition"] for _, _, outcome in outcomes)
    assert all(outcome.reason_code == contract["acceptance"]["expected_reason_code"] for _, _, outcome in outcomes)
