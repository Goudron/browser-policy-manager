...
def test_controller_service_binds_opaque_browser_session_to_context_and_sources() -> None:
    ...
    assert service.source(request_id="request-1", source_id="source-1", session_id="browser-session") is None
    try:
...
def test_service_derives_web_mode_from_server_state_and_clear_does_not_reset_it() -> None:
    ...
    service.ask(
        request=ConversationRequest(
            "en", "Next question", web_mode="local_only", web_tab_id=TAB_A
        ),
        session_id="browser-session",
    )
    ...
    service.ask(
        request=ConversationRequest(
            "en", "Other tab", web_mode="request_web", web_tab_id=TAB_B
        ),
        session_id="browser-session",
    )
    assert controller.submitted[-1].web_mode == "local_only"
    ...
    assert service.web_mode_status(
        locale="en", session_id="browser-session", tab_id=TAB_A
    ).enabled is True
    assert service.web_mode_status(
        locale="en", session_id="browser-session", tab_id=TAB_B
    ).enabled is False
...
def test_service_keeps_context_and_clear_private_to_one_tab_and_locale() -> None:
    ...
    assert service.clear(
        session_id="browser-session", locale="en", tab_id=TAB_A
    ) is True
    try:
...
def test_training_service_is_session_private_and_only_returns_the_release_notice() -> None:
    ...
    assert service.web_mode_status(locale="ru", session_id="browser-a").reason_code == "assistant_web_disabled"
    assert service.set_web_mode(
        locale="ru", session_id="browser-a", enabled=True
    ).available is False
    assert service.source(request_id=admission.request_id, source_id="anything", session_id="browser-a") is None
...
def test_controller_service_fails_closed_for_admission_web_state_and_source_resolution() -> None:
    ...
    assert service.web_mode_status(locale="en", session_id="browser").reason_code == "assistant_web_disabled"
    assert service.set_web_mode(locale="en", session_id="browser", enabled=True).reason_code == "assistant_web_disabled"

    available = WebEvidenceModeStore(
        configuration=WebEvidenceConfiguration(True, "token")
    )
    checked = _service(_Controller(), ConversationContextStore(), available)
    assert checked.web_mode_status(locale="en", session_id="browser", tab_id="bad").reason_code == "assistant_web_mode_invalid"
    assert checked.set_web_mode(
        locale="en", session_id="browser", enabled=True, tab_id="bad"
    ).reason_code == "assistant_web_mode_invalid"
    ...
    class InvalidMode:
        ...
        def set_enabled(self, **_: object) -> object:
            return type("Bad", (), {"enabled": 1, "available": True, "reason_code": "", "state_epoch": -1})()

        def is_enabled(self, **_: object) -> bool:
            return False

    invalid = _service(_Controller(), ConversationContextStore(), InvalidMode())  # type: ignore[arg-type]
    assert invalid.web_mode_status(locale="en", session_id="browser").reason_code == "assistant_web_mode_unavailable"
    assert invalid.set_web_mode(locale="en", session_id="browser", enabled=True).reason_code == "assistant_web_mode_unavailable"

    controller = _Controller()
    unresolved = ControllerDocumentationAssistantService(
        controller=controller,  # type: ignore[arg-type]
        context_store=ConversationContextStore(),
        diagnostics=_Diagnostics(AssistantDiagnostics("offline", False, True, True, True, True, "ready", "idle", "none", "local_only")),  # type: ignore[arg-type]
        bpm_version="0.9.3",
        source_resolver=lambda _: None,
    )
    assert unresolved.status(locale="en", session_id="browser").action_key == "assistant_manage_model"
    admitted = unresolved.ask(request=ConversationRequest("en", "question"), session_id="browser")
    assert admitted.request_id == "request-1"
    assert unresolved.source(request_id="request-1", source_id="source-1", session_id="browser") is None
    assert unresolved.source(request_id="unknown", source_id="source-1", session_id="browser") is None
...
def test_controller_service_handles_context_races_and_all_ownership_fallbacks() -> None:
    with pytest.raises(ValueError, match="bpm_version"):
        ControllerDocumentationAssistantService(
            ...
            context_store=ConversationContextStore(),
            diagnostics=_Diagnostics(AssistantDiagnostics("ready", True, True, True, True, True, "ready", "idle", "none", "local_only")),  # type: ignore[arg-type]
            bpm_version="",
            source_resolver=lambda _: None,
        )
    ...
    assert accepted.source(request_id="request-1", source_id="missing", session_id="browser") is None
    ...
    assert racing.ask(request=ConversationRequest("en", "raced"), session_id="browser").request_id is None
...
def test_safe_web_mode_status_keeps_available_disabled_state() -> None:
    status = ControllerDocumentationAssistantService._safe_web_mode_status(
        type("State", (), {"enabled": False, "available": True, "reason_code": "web_ready", "state_epoch": 4})(),
        "en",
    )
