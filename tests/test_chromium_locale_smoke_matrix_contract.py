...
AUDIT_DOC_PATH = REPO_ROOT / "docs" / "chromium_locale_smoke_matrix_audit_2026-05-30.md"
...
def test_chromium_locale_smoke_matrix_audit_records_gloc_601_scope():
    ...
    for locale in ("de", "zh-CN", "fr", "es-ES"):
        assert f"| `{locale}` | covered | covered | covered | covered |" in audit_doc
    for route_name in ("Library", "Guided editor", "All settings", "JSON editor"):
        assert route_name in audit_doc


def test_chromium_locale_smoke_matrix_browser_test_covers_new_locales_and_routes():
    ...
    assert 'target_locales = ("de", "zh-CN", "fr", "es-ES")' in browser_test_source
