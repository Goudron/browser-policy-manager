...
def test_cis_documentation_inventory_is_current_and_complete():
    ...
    assert inventory["backlog_item"] == "BPM090-M2-04"
...
def test_cis_documentation_inventory_closes_publication_dispositions():
    ...
    planned = [
        entry for entry in recommendations if entry["publication_disposition"] == "planned-dita-topic"
    ]
    provenance_only = [
        entry
    ...
    assert {
        (entry["recommendation_id"], entry["mapping_status"])
        for entry in provenance_only
    } == {
...
def test_cis_documentation_inventory_targets_firefox_documentation_ids():
    ...
    assert all(target["target_doc_id"].startswith("fx-policy-") for target in targets if target["kind"] == "policy")
