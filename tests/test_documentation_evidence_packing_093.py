...
def _evidence(
    ...
    *,
    ...
    return RetrievedEvidence(
        ...
        score=score,
        citation=LocalCitation(f"topic:{topic_id}", f"/help/en/user/{topic_id}.html", topic_id, "root"),
    )
...
def test_packer_bounds_context_deduplicates_citations_and_preserves_source_order() -> None:
    ...
    assert [citation.citation_id for citation in pack.citations] == ["topic:topic-a", "topic:topic-b"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_result(_evidence("ragc-v1:en:low:root:0", topic_id="low", score=0.44)), True, "abstain", "low_confidence"),
        (
            _result(
        ...
        (_result(_evidence("ragc-v1:en:ok:root:0", topic_id="ok", score=0.90)), False, "abstain", "scope_not_admitted"),
    ],
)
def test_packer_stops_before_generation_for_empty_low_confidence_or_unadmitted_evidence(
    ...
    pack = EvidencePacker(_exact_counter, bpm_version="0.9.3").pack(result, scope_admitted=scope_admitted)
...
def test_packer_rejects_stale_contradictory_and_over_budget_evidence() -> None:
    ...
    stale = packer.pack(_result(_evidence("ragc-v1:en:stale:root:0", topic_id="stale", score=0.90, version="0.9.2")), scope_admitted=True)
    contradictory = packer.pack(
        _result(
            _evidence("ragc-v1:en:same:root:0", topic_id="same", score=0.90, source_sha256="a" * 64),
            _evidence("ragc-v1:en:same:root:1", topic_id="same", score=0.60, ordinal=1, source_sha256="b" * 64),
        ),
        scope_admitted=True,
    )
    over_budget = EvidencePacker(_exact_counter, bpm_version="0.9.3", maximum_context_tokens=1).pack(
        _result(_evidence("ragc-v1:en:large:root:0", topic_id="large", score=0.90)), scope_admitted=True
    )
...
def test_packer_rejects_an_invalid_exact_token_counter_or_configuration() -> None:
    ...
    with pytest.raises(EvidencePackingError, match="token_counter_returned_invalid_count"):
        packer.pack(_result(_evidence("ragc-v1:en:ok:root:0", topic_id="ok", score=0.90)), scope_admitted=True)
...
def test_packer_limits_selected_chunks_and_skips_oversized_later_context() -> None:
    limited = EvidencePacker(
        _exact_counter, bpm_version="0.9.3", maximum_chunks=1
    ).pack(
        _result(
            _evidence("ragc-v1:en:a:root:0", topic_id="a", score=0.90),
    ...
    candidates = tuple(
        replace(
            _evidence(f"ragc-v1:en:large-{index}:root:0", topic_id=f"large-{index}", score=0.90 - index / 100),
            text=large_text,
        )
