...
def test_validator_accepts_only_structured_own_word_sections_with_approved_local_citations() -> None:
...
def test_nonanswer_cannot_carry_model_sections_and_unsafe_source_abstains() -> None:
    ...
    clarify = validator(
        _result({"disposition": "clarify", "sections": []}), _evidence()
    )
...
def test_validator_never_exposes_a_rewrite_candidate_for_other_invalid_output() -> None:
    ...
    assert validator.candidate_for_excessive_quote(
...
def test_validator_rejects_invalid_section_bounds_answer_length_and_citation_identity() -> None:
    validator = GenerationResponseValidator()
    evidence = _evidence()
    assert validator._assess_answer_sections([], {"topic:alpha": evidence.citations[0]}, evidence) is None
    assert validator._assess_answer_sections(
...
def test_quote_check_skips_evidence_not_supporting_the_section() -> None:
    ...
    assert GenerationResponseValidator._contains_excessive_evidence_quote(
