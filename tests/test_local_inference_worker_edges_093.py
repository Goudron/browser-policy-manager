...
def test_default_installation_health_timing_and_state_guards(
    ...
    assert LocalInferenceWorker.for_default_installation(enabled=True).health().state == "not-installed"
...
def test_startup_and_serialization_edge_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    worker = _worker(tmp_path)
    for request, code in (
        (InferenceRequest("xx", "question", "{}"), "assistant_unsupported_locale"),
        (InferenceRequest("en", "", "{}"), "assistant_invalid_question"),
        (InferenceRequest("en", "question", "{}", (("bad", "text"),)), "assistant_invalid_dialogue"),
    ...
    monkeypatch.setattr(worker, "_assert_execution_paths", lambda: None)
    monkeypatch.setattr(module, "extract_verified_worker_bundle", lambda *_: (_ for _ in ()).throw(OSError("no")))
    with pytest.raises(WorkerUnavailable, match="assistant_start_failed"):
        worker._ensure_started_locked()
    worker = _worker(tmp_path / "runtime-error")
    monkeypatch.setattr(worker, "_assert_execution_paths", lambda: None)
    monkeypatch.setattr(module, "extract_verified_worker_bundle", lambda *_: (_ for _ in ()).throw(RuntimeError("bad")))
    with pytest.raises(WorkerUnavailable, match="assistant_start_failed"):
        worker._ensure_started_locked()
...
def test_worker_path_safety_and_hidden_reasoning_are_fail_closed(
    ...
    assert LocalInferenceWorker._strip_hidden_reasoning("<think>x</think> answer <think>y") == "answer y"
...
def test_worker_terminal_process_and_remaining_os_fallbacks(
    ...
    monkeypatch.setattr(worker, "_ensure_started_locked", lambda: setattr(worker, "_process", exited))
