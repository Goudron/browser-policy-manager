...
def _write_schema(path, *, channel: str = "release-148") -> None:
    payload = {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "title": "Release Policies 148.0",
        "type": "object",
...
def _write_custom_schema(path, properties: dict[str, object], *, channel: str = "release-148") -> None:
    payload = {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "title": "Release Policies 148.0",
        "type": "object",
...
def test_get_schema_path_missing_file_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(FileNotFoundError):
        service._get_schema_path("release-148")


def test_load_policy_schema_and_get_definition(tmp_path, monkeypatch):
    release_file = tmp_path / "firefox-release-148.json"
    _write_schema(release_file)

    monkeypatch.setattr(service, "SCHEMAS_DIR", tmp_path)
    service.load_policy_schema.cache_clear()

    schema = service.load_policy_schema("release-148")
    definition = service.get_policy_definition("release-148", "DisableTelemetry")
    missing = service.get_policy_definition("release-148", "NoSuchPolicy")

    assert schema.channel == "release-148"
    ...
    cached = service.load_policy_schema("release-148")
    assert cached.version == "148.0"
...
def test_load_policy_schema_uses_safe_fallback_ui_metadata(tmp_path, monkeypatch):
    ...
    definition = service.load_policy_schema("release-148").get_policy("CustomPortal")
...
def test_real_policy_schemas_expose_ui_metadata_for_every_policy():
    service.load_policy_schema.cache_clear()
    section_ids = {section.id for section in service.load_policy_schema("release-148").ui_sections}
