...
class _Session:
    def __init__(self, providers: list[str] | None = None, names: tuple[str, ...] = ("input_ids", "attention_mask", "token_type_ids")) -> None:
    ...
    def run(self, unused: object, feeds: dict[str, np.ndarray]) -> list[np.ndarray]:
        del unused
        return [np.ones((len(feeds["input_ids"]), 2, e5_runtime.VECTOR_DIMENSION), dtype=np.float32)]
...
def test_verify_model_directory_requires_regular_checksum_pinned_files(
    ...
    model.mkdir()
    monkeypatch.setattr(e5_runtime, "MODEL_FILES", {"config.json": hashlib.sha256(b"ok").hexdigest()})
...
@pytest.mark.parametrize(
    ("session", "message"),
    [(_Session(providers=["CUDAExecutionProvider"]), "unavailable"), (_Session(names=("input_ids",)), "unavailable")],
)
