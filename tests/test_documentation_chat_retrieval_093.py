...
def _create_active_generation(tmp_path: Path, version: str = "0.9.3") -> Path:
    ...
    for locale in SUPPORTED_LOCALES:
        ...
        _write_json(metadata_path, {"locale": locale, "chunks": [_chunk(locale, 0, version), _chunk(locale, 1, version)]})
        ...
        _write_json(
            manifest_path,
            {
                ...
                "compatibility_key": {
                    ...
                    "embedding_model_revision_or_checksum": "f60256a833caee5c75a3903e589116752ee016ca7bc16f9b96e4db09984c5703",
                ...
                "files": {
                    "vectors.f32": {"byte_count": vector_path.stat().st_size, "sha256": _sha256(vector_path)},
                    "chunks.json": {"byte_count": metadata_path.stat().st_size, "sha256": _sha256(metadata_path)},
                },
            },
        )
        entries.append({"locale": locale, "manifest_sha256": _sha256(manifest_path), "chunk_count": 2})
    ...
    _write_json(index_root / "active-generation.json", {"generation_id": generation_id, "manifest_sha256": _sha256(root_manifest)})
    return index_root
...
def test_retrieval_uses_only_the_requested_locale_and_keeps_local_citations(tmp_path: Path) -> None:
    ...
    assert all(candidate.citation.published_url.startswith("/help/ru/") for candidate in russian.candidates)
...
def test_retrieval_applies_guide_filter_and_returns_an_explicit_empty_evidence_set(tmp_path: Path) -> None:
    retriever = ExactLocaleRetriever(_create_active_generation(tmp_path), bpm_version="0.9.3")

    filtered = retriever.retrieve(locale="en", query_vector=_query(0), guide_ids={"administrator-guide"})
...
def test_retrieval_rejects_stale_tampered_or_invalid_requests_without_fallback(tmp_path: Path) -> None:
    stale = ExactLocaleRetriever(_create_active_generation(tmp_path / "stale", version="0.9.2"), bpm_version="0.9.3")
...
def test_retrieval_low_level_guards_reject_unsafe_files_vectors_and_citations(tmp_path: Path) -> None:
...
def test_retrieval_rejects_invalid_limit_and_generation_manifest_layers(tmp_path: Path) -> None:
    invalid_limit = ExactLocaleRetriever(_create_active_generation(tmp_path / "limit"), bpm_version="0.9.3")
    ...
    manifest_path = root / "generations" / "raggen-v1-0123456789abcdef0123" / "generation-manifest.json"
    manifest = _read_json(manifest_path)
    manifest["storage_backend_id"] = "unexpected"
    _write_json(manifest_path, manifest)
    _refresh_active_pointer(root)
    with pytest.raises(RetrievalUnavailable, match="incompatible_generation"):
        ExactLocaleRetriever(root, bpm_version="0.9.3").retrieve(locale="en", query_vector=_query(0))
...
def test_retrieval_rejects_invalid_locale_artifacts_after_verified_manifest_chain(
    ...
    locale_manifest = root / "generations" / "raggen-v1-0123456789abcdef0123" / "en" / "manifest.json"
