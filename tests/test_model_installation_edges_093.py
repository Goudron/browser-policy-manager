...
def test_installation_inspection_and_store_guards(
    tmp_path: Path, artifact: tuple[installation.ModelArtifact, bytes], monkeypatch: pytest.MonkeyPatch
) -> None:
    _, payload = artifact
...
def test_install_local_remove_and_staging_edge_cases(
    ...
    installer = _installer(tmp_path)
    with pytest.raises(installation.ArtifactVerificationError, match="unsafe_local_artifact_source"):
        installer.install_local(tmp_path / "missing", confirmed=True)
    wrong = tmp_path / "wrong.gguf"
    wrong.write_bytes(b"wrong")
    with pytest.raises(installation.ArtifactVerificationError, match="local_artifact_size_mismatch"):
...
def test_download_copy_promotion_and_removal_primitives(
    ...
    progress: list[int] = []
    assert installer._write_download(Response(), part, 0, lambda item: progress.append(item.downloaded_bytes), None) == len(payload)  # type: ignore[arg-type]
...
def test_locks_progress_parser_and_cli_dispatch(
    ...
    monkeypatch.setattr(
        Path,
        "unlink",
        lambda path, *args, **kwargs: (_ for _ in ()).throw(FileNotFoundError())
        if path == lock_path
    ...
    monkeypatch.setattr(installation, "get_settings", lambda: SimpleNamespace(DATA_DIR=tmp_path, AI_MODEL_DOWNLOAD_TIMEOUT_SECONDS=1))
...
def test_remaining_model_lifecycle_fail_closed_branches(
    ...
    copied: list[int] = []
    installer._copy_local_artifact(unsafe, copy_part, lambda item: copied.append(item.downloaded_bytes), None)
    ...
    with pytest.raises(installation.ArtifactVerificationError, match="unsafe_existing_model_directory"):
        installer._promote_staging()
    installer.model_dir.unlink()
    monkeypatch.setattr(
        installer,
