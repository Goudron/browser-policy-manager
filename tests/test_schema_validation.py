...
The tests assert:
- Schemas for "esr-140" and "release-145" are loadable.
...
def test_available_profiles_scope():
    ...
    assert set(profiles.keys()) == {"esr-140", "release-145"}
    assert profiles["esr-140"].endswith("firefox-esr140.json")
    assert profiles["release-145"].endswith("firefox-release.json")


@pytest.mark.parametrize("profile", ["esr-140", "release-145"])
