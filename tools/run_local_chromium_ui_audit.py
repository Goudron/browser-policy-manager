...
class UiAuditRunner:
    ...
    def set_profile_metadata(self, driver: WebDriver) -> None:
        driver.execute_script(
            ...
            if (profileTypeEl && Array.from(profileTypeEl.options).some((option) => option.value === "release-152")) {
                profileTypeEl.value = "release-152";
    ...
    def debug_save_request(self, ui_state: dict[str, Any], flags_payload: dict[str, Any]) -> dict[str, Any]:
        payload = {
            ...
            "schema_version": ui_state.get("profileType") or "release-152",
            "flags": flags_payload,
        }
        response = requests.post(
