...
def build_inventory() -> dict[str, Any]:
    ...
    for channel in SCHEMA_CHANNELS:
        ...
        channel_metadata[channel.value] = {
            ...
            "schema_source_provenance": raw_schema.get("x-bpm-source-provenance", {}),
...
def render_summary(inventory: dict[str, Any]) -> str:
    ...
    esr_115 = set(by_id) & {
        policy_id for policy_id, policy in by_id.items() if "esr-115.39" in policy["channels"]
    }
    esr_140 = {
    ...
    return f"""# Firefox Policy Documentation Inventory (BPM 0.9.5.1)
...
153 artifacts but not ESR 115.39; {len(release - esr_140)} policies are available only
...
default are the explicit `esr-153.0` catalog roles; ESR 115.39 and ESR 140.13 remain
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the BPM 0.9.5.1 Firefox policy documentation inventory."
    )
    parser.add_argument(
        "--check",
        action="store_true",
