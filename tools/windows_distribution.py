#!/usr/bin/env python3
"""Build, smoke-test, and stage the BPM 0.9.6 native Windows x64 MSI.
...
def load_target() -> WindowsTarget:
    ...
    if payload.get("target_bpm_version") != TARGET_VERSION:
        raise WindowsDistributionError("Windows target manifest must match BPM 0.9.6")
    ...
    if target.artifact != "browser-policy-manager-0.9.6-windows-x64.msi":
        raise WindowsDistributionError(
            "Windows MSI artifact name is not the BPM 0.9.6 release name"
        )
...
def stage_release_assets(target: WindowsTarget) -> None:
    """Add the verified signed MSI to the shared BPM 0.9.6 release store."""
