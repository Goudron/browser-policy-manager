...
- [app/templates/profiles_json.html](../app/templates/profiles_json.html)
...
Right now `/profiles/{id}/json` combines:
...
- `/profiles/{id}/json` is not part of the final product vocabulary.
  ...
  - New UI, new links, tests, and documentation should stop treating `/json` as canonical.
...
- All later tasks in this backlog should treat `/settings` and `/json` as the destination model, even if `/json` still exists temporarily in code during migration.
...
- Kept the current `/profiles/{id}/json` shell transitional for now, because deeper advanced/settings/json separation belongs to later workstreams in this backlog.
...
- Introduced explicit mode entrypoints for the remaining editor routes:
  ...
  - [app/static/profiles_json.js](../app/static/profiles_json.js)
...
- Extracted a shared editor chrome partial in [app/templates/profiles/_page_editor_chrome.html](../app/templates/profiles/_page_editor_chrome.html) and mounted it from the shared guided/json shell.
...
- Removed lifecycle / risk actions from editor mode shells:
  ...
  - `hard-delete`
  - `reset-library`
- Preserved save-conflict handling and status summary in the deck, because they still belong to active editing work and are shared by the current guided/json runtime.
...
- the guided route no longer knows about an internal guided/json switch
...
- Updated mode links so the explicit "Advanced settings" entry now opens `/settings`, while the transitional JSON link still targets `/json`.
...
- current `/profiles/{id}/json` is kept only as a compatibility redirect to `/profiles/{id}/json`
...
- Kept `/profiles/{id}/json` as a temporary compatibility route, but changed it to a `307` redirect that preserves safe `return` and `focus` query params when forwarding to `/profiles/{id}/json`.
...
- Replaced the transitional JSON shell include with a dedicated [profiles/_json_shell.html](../app/templates/profiles/_json_shell.html) composition, so `/profiles/{id}/json` no longer rides through the old guided/json shell stack.
