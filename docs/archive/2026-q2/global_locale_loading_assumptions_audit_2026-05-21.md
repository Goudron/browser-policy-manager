...
| `tests/contract/ui/localization/test_ui_runtime_i18n_contract.py` | Runtime key existence previously covered only `en` and `ru`. | Updated in `GLOC-401` to scan JavaScript and templates for runtime locale-key shapes and require every referenced key in all active catalog locales from the locale matrix. |
| `tests/integration/locale/test_ru_locale_quality.py` | Accidental-English and Mozilla terminology guards exist only for Russian. | Extend in `GLOC-306` with locale-specific allowlists for the new locales after terminology QA begins. |
