...
3. Advanced is a separate destination.
   The JSON editor should open as its own route/tab, not as an embedded secondary UI inside the visual wizard.
...
- `WS-04` Separate JSON editor route.
...
- Load the target profile into the JSON editor.

Acceptance:

- Direct navigation opens the JSON editor for the profile.
- Unknown profile IDs produce a clear not-found or error state.
- Existing JSON editor save/validate/download behavior still works.

Implementation notes:

- Added `/profiles/{profile_id}/json` with profile lookup and 404 handling.
- Added `profiles_json.html` as a separate template entry point with `data-profiles-template-kind="advanced"`.
...
### `WS-B02` Remove Default Inline JSON Editor From Visual Wizard

Status: Done

Scope:

- Stop rendering the full JSON editor by default inside the visual wizard.
...
- The advanced content grid is hidden on visual editor routes and remains visible on `/profiles/{id}/json`.
- The guided advanced handoff now links to `/profiles/{id}/json` for saved profiles and is disabled for unsaved drafts.
...
- JSON editor shows `Return to visual editor` when `return` is supplied.
...
- Handoff from visual wizard opens `/profiles/{id}/json`.
...
- Tests fail if visual wizard still renders the full JSON editor by default.
...
- If no expected revision is supplied during the transition period, existing clients keep working. After visual and JSON editors are wired, UI saves must always send the expected revision.
...
### `WS-C05` Wire Revision Token Into JSON Editor Saves

Status: Done

Scope:

- Apply the same revision protection to JSON editor saves.

Acceptance:

- JSON editor saves include revision protection.
- Stale advanced save does not overwrite a newer visual editor save.

Implementation notes:

- JSON editor route already uses the shared `saveCurrent` implementation, so it now sends `expected_revision` through the `WS-C04` save contract.
...
- Visual editor and JSON editor use consistent conflict language.
...
- Apply to visual and JSON editors.
...
6. **Advanced link**
   ...
   - Link to `/profiles/{id}/json` with a focus target when the profile is saved.
...
- Removed the general advanced preferences handoff block from step 2; preference editing remains available through search/json routes instead of the default step body.
...
- Replace `JSON editor` primary labels.
- Normalize English and Russian labels for advanced/full JSON/export semantics.

Acceptance:

- UI uses `Full policies.json`, `JSON editor`, `Полный policies.json`, or `Расширенный редактор`.
- Export copy clearly distinguishes included, not included, and manual-review states.

Implementation notes:

- Replaced the older `JSON editor` / `JSON editor` labels in visible UI copy with `JSON editor` / `Расширенный редактор`.
...
- Added placeholders for the JSON editor guide, lower-level policy reference, and final-step boundary notes.
...
- Finished the advanced-workflow RU terminology cleanup so user-facing copy no longer mixes in the earlier `JSON editor` wording.
...
- Open visual editor and JSON editor routes for the same profile.
...
- Added a browser-facing smoke regression that opens the same saved profile through `/profiles/{id}/edit` and `/profiles/{id}/json`.
...
2. Milestone B: JSON editor as a separate destination.
...
- Visual editor and JSON editor use explicit saves and detect stale revisions.
...
- Browser-level regression coverage protects the main create/edit/json/conflict/export workflows.
