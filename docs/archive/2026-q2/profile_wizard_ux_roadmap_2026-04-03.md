...
1. Coverage transparency in guided mode.
   Show what the current step covers directly, what remains in the JSON editor, and what will still be preserved.
...
2. Guided-to-advanced handoff.
   Replace generic JSON-editor entry points with targeted jumps into the exact advanced area that still needs attention.
...
- JSON editor remains available, but not required for the common path.
...
3. Dock, details, and advanced copy pass.
   Simplified heavy editor/document wording and made JSON-editor labels clearer.
...
11. JSON editor empty states and first actions.
    Added neutral JSON-editor onboarding with direct actions for profile details, full document editing, and validation.

12. JSON editor utility hierarchy.
    Reframed JSON editor around `details -> full document -> validation/downloads` and separated saved-file downloads more clearly.

13. JSON editor review visibility.
...
- clearer JSON-editor onboarding
...
1. `PR0.1` unified interface vocabulary.
   Keep terms such as `profile`, `draft`, `saved profile`, `Guided setup`, and `JSON editor` consistent across copy, status surfaces, and empty states.
...
10. `PR3.1` more targeted review-to-advanced jumps. Completed.
    Take users from review and drill-down surfaces into the most relevant advanced area instead of into JSON editor in general.

11. `PR3.2` simpler advanced entry for first-time users.
    Further reduce the chance that someone enters JSON editor too early when guided mode already covers the task.
...
5. Stronger export explainability.
   Make the finish step show not only readiness, but also `what changed since last save`, `what this handoff contains`, and `what still lives only in JSON editor`.
...
7. Coverage prioritization by external enterprise evidence.
   ...
   Completed on 2026-04-04 via [`docs/archive/2026-q2/profile_wizard_coverage_priority_register_2026-04-04.md`](./profile_wizard_coverage_priority_register_2026-04-04.md), which records cluster priority, wizard-fit decisions, and the recommended execution order.
...
- `PB2.7` is tracked in [`docs/archive/2026-q2/profile_wizard_coverage_priority_register_2026-04-04.md`](./profile_wizard_coverage_priority_register_2026-04-04.md) and should be the default planning input whenever future guided coverage is discussed.
...
- JSON editor remains deliberate and useful, not a dumping ground.
