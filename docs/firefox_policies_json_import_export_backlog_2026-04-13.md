...
7. Update wizard download-link JavaScript.
   ...
   - guard any legacy internal download links that may remain in advanced mode;
...
9. Decide the advanced download policy.
   ...
   Problem:
   Advanced mode currently exposes internal JSON/YAML downloads. These may be useful for debugging or compatibility, but they conflict with a strict "input and output are Firefox `policies.json`" reading.
...
### P1: Advanced editor full-document mode

11. Decide whether the advanced editor should show full `policies.json`.

    Problem:
    The advanced editor currently edits internal `flags` directly. Moving it to full `policies.json` would align the whole product model, but it touches many wizard sync functions that expect policy keys at the editor root.

    Decision:
    Proceed with full-document mode. The advanced editor should display Firefox `policies.json`, while save and guided synchronization continue to use normalized internal flags through shared adapters.
...
12. Revisit full-document advanced editor mode as a separate follow-up.
...
- The advanced editor full-document move is handled through shared frontend adapters so wizard modules keep using normalized flags while users see Firefox `policies.json`.
