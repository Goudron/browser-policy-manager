# BPM 0.9.5 Native macOS DMG Distribution Contract

Status: test-delivery contract; Apple-signed production publication deferred.

## Scope

BPM 0.9.5 has two native DMG test targets for macOS 14 and later:

| Target | Runner used for assembly | Artifact |
| --- | --- | --- |
| Intel (`x64`) | `macos-15-intel` | `browser-policy-manager-0.9.5-macos-x64.dmg` |
| Apple Silicon (`arm64`) | `macos-14` | `browser-policy-manager-0.9.5-macos-arm64.dmg` |
...
copy-to-Applications shortcut, and a test readme. The application bundle is a
...
item, privileged helper, or production-deployment claim in this test contour.
...
assets. No test DMG may enter it or be published. A production DMG requires a
separate protected Apple Developer workflow that records a valid Developer ID
signature, submits the exact DMG to Apple notarization, staples its ticket, and
only then stages its checksum and receipts for the GitHub Release `v0.9.5`.
