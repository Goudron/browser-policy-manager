...
| `README.md` and `tests/integration/app/test_current_version_surfaces.py` | Current shipped-product copy accurately says search is deterministic, local, offline-capable, and non-AI. | **Replace** in M12-06 only after final shipped behavior is proven. | README remains durable current-state copy; it may not advertise target-only AI/RAG work. |
